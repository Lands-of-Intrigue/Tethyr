...
/*  Ability: Strength.
    ...
    else if(!bRanged)
    {
        ...
        if(GetIsUndead(GlobalMeleeTarget) == TRUE){nTargetCreatureRace = RACIAL_TYPE_UNDEAD;}
...
int AI_AttemptConcentrationCheck(object oTarget)
{
    ...
    // DEFENSIVE CASTING:
    // This stops AOO's (normal damage concentration checks from attacks still applies)
    // This is useful for stopping a lot of extra attacks.
    ...
    // always turn it on.
     ((GetSkillRank(SKILL_CONCENTRATION) >= 24) ||
    // Else we'll turn it on based on our class level. Over class level
    (((GetSkillRank(SKILL_CONCENTRATION) >= GlobalOurHitDice + 6) ||
    ...
    // them will be pretty high otherwise. (ONLY if they are a comparable level!)
     ((GlobalMeleeAttackers >= 4 && GlobalAverageEnemyBAB + 15 >= GlobalOurAC) ||
      (GlobalMeleeAttackers >= 7))))))
    {
        // Turn it on
...
int AI_AttemptHealingSelf()
{
    ...
    if(GetIsUndead(OBJECT_SELF) == TRUE){nRace = RACIAL_TYPE_UNDEAD;}
...
int AI_AttemptCureCondition()
{
    ...
    {
        // Don't target undead
        if(GetRacialType(oHeal) != RACIAL_TYPE_UNDEAD && GetIsUndead(oHeal) == FALSE)
        {
       ...
       !GetSpawnInCondition(AI_FLAG_OTHER_COMBAT_NO_CURING, AI_OTHER_COMBAT_MASTER))
    {
        ...
        else if(GlobalValidSeenAlly)
        {
            ...
            if(nChosenPercentHitPoints > 0)
            {
                ...
                if(GetIsUndead(oAllyToHeal) == TRUE){nAllyHealRace = RACIAL_TYPE_UNDEAD;}
        ...
        GlobalNormalSpellsNoEffectLevel < 8)// Mass heal = 8.
    {
           ...
           GlobalSpellTargetRace != RACIAL_TYPE_INVALID)
        {
            ...
            if(GlobalNormalSpellsNoEffectLevel < 6 &&
              ((GetRacialType(OBJECT_SELF) != RACIAL_TYPE_UNDEAD && GetIsUndead(OBJECT_SELF) != TRUE )||
...
//::22222222222222222222222222222222222222222222222222222222222222222222222222*/
    ...
    if(bFirstRunThrough)
    {
        ...
        if(!AI_GetAIHaveSpellsEffect(GlobalHasNaturalACSpell))
        {
            ...
            if(GetRacialType(OBJECT_SELF) == RACIAL_TYPE_UNDEAD || GetIsUndead(OBJECT_SELF) == TRUE)
            {
            ...
            else if(GetRacialType(GlobalBuffAlly) == RACIAL_TYPE_UNDEAD || GetIsUndead(GlobalBuffAlly) == TRUE)
            {
                // Stone bones - cast if we do not have a natural armor AC spell.
...
//::////////////////////////////////////////////*/
    ...
    if(GetIsUndead(GlobalSpellTarget) == TRUE){GlobalSpellTargetRace = RACIAL_TYPE_UNDEAD;}
    // Range
      ...
      !SRA)// Spell ranged attacking = AI_FLAG_COMBAT_LONGER_RANGED_SPELLS_FIRST
    {
           ...
           !GetLocalTimer(AI_TIMER_EMPATHY))
        {
               ...
               GetSpawnInCondition(AI_FLAG_OTHER_COMBAT_FORCE_EMPATHY, AI_OTHER_COMBAT_MASTER))
            {
                ...
                if(GetIsUndead(GlobalMeleeTarget) == TRUE){iRace = RACIAL_TYPE_UNDEAD;}
