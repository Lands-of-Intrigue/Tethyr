...
void TrapDoElectricalDamage(int ngDamageMaster, int nSaveDC, int nSecondary,object oTarget = OBJECT_INVALID);
...
void doRewardForCure(object oCaster, object oTarget, int nDamageLVL)
{
    ...
    if (nDamageLVL == 4)
    {
        ...
        {
            GiveXPToCreature(oCaster, 5);
            return;
        }
    }
    else if (nDamageLVL == 3)
    {
        if (GetHitDice(oCaster) <=10)
        {
            GiveXPToCreature(oCaster, 20);
            return;
        }
        else
        {
    ...
    else if (nDamageLVL == 2)
    {
        if (GetHitDice(oCaster) <=10)
        {
            GiveXPToCreature(oCaster, 15);
            return;
        }
        else
        {
    ...
    else if (nDamageLVL == 1)
    {
        if (GetHitDice(oCaster) <=10)
        {
            GiveXPToCreature(oCaster, 10);
            return;
        }
        else
        {
...
void spellsCure(int nDamage, int nMaxExtraDamage, int nMaximized, int vfx_impactHurt, int vfx_impactHeal, int nSpellID)
{
    ...
    effect eHeal, eDam;
    int nStartHP  = GetCurrentHitPoints(oTarget);
    int nBarelyInjured = FloatToInt(GetMaxHitPoints(oTarget)*0.95);
    int nInjured = FloatToInt(GetMaxHitPoints(oTarget)*0.75);
    int nBadlyWounded = FloatToInt(GetMaxHitPoints(oTarget)*0.5);
    int nNearDeath = FloatToInt(GetMaxHitPoints(oTarget)*0.25);




    int nExtraDamage = TE_GetCasterLevel(OBJECT_SELF,GetLastSpellCastClass()); // * figure out the bonus damage
    ...
    if (nMetaMagic == METAMAGIC_EMPOWER || GetHasFeat(FEAT_HEALING_DOMAIN_POWER) || (TE_GetDeity(OBJECT_SELF) == TE_GetDeity(oTarget)))
    {
    ...
    else if(GetRacialType(oTarget) == RACIAL_TYPE_CONSTRUCT || GetRacialType(oTarget) == RACIAL_TYPE_ELEMENTAL)
    {
        ApplyEffectToObject(DURATION_TYPE_INSTANT,EffectVisualEffect(vfx_impactHeal),oTarget);
    }
    ...
    if (!GetIsPC(oTarget))
    {
        if (nStartHP <= nNearDeath && (GetCurrentHitPoints(oTarget)>= nBarelyInjured))// below 25%
        {
            doRewardForCure(OBJECT_SELF, oTarget, 4);
            return;
        }
        else if (nStartHP <= nBadlyWounded && (GetCurrentHitPoints(oTarget)>= nBarelyInjured))// below 50%
        {
            doRewardForCure(OBJECT_SELF, oTarget, 3);
            return;
        }
        else if (nStartHP <= nInjured && (GetCurrentHitPoints(oTarget)>= nBarelyInjured)) //below 75%
        {
            doRewardForCure(OBJECT_SELF, oTarget, 2);
            return;
        }
        else if (nStartHP <= nBarelyInjured && (GetCurrentHitPoints(oTarget)>= nBarelyInjured)) // below 95%
        {
            doRewardForCure(OBJECT_SELF, oTarget, 1);
            return;
        }
    }
    else
...
{
  int n = TE_GetCasterLevel(OBJECT_SELF,GetLastSpellCastClass());
