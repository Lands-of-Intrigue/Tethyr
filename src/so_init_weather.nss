...
void main()
{
    WriteTimestampedLogEntry("<O> Setting up weather.");
    ...
    WriteTimestampedLogEntry("<-> Temperature: "+IntToString(FloatToInt(fTemperatureAim)));
    WriteTimestampedLogEntry("<-> Humidty: "+IntToString(FloatToInt(fHumidityAim)));

    //ExecuteScript("te_weathpseud", oModule);
}
