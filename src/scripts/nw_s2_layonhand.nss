...
void main()
{
    ...
    object oItem = GetItemPossessedBy(OBJECT_SELF,"PC_Data_Object");
    int nPiety = GetLocalInt(oItem,"nPiety");

    if(nPiety == 0 && nLevel >= 1)
    {
        return;
    }
    else if(nLevel >= 1 && nPiety != 0 && nPiety <= 45)
    {
        ...
        {
            SendMessageToPC(OBJECT_SELF,"Your actions have increased your piety to "+IntToString(nPiety+5)+ " out of 100.");
