...
// End of Spell Cast Hook
    ...
    if(!GetIsReactionTypeFriendly(oTarget))
    {
       ...
       // GZ: * GetSpellCastItem() == OBJECT_INVALID is used to prevent feedback from showing up when used as OnHitCastSpell property
        ...
        {
            //SR and Saves
            if(!MyResistSpell(OBJECT_SELF, oTarget) && !/*Fort Save*/ MySavingThrow(SAVING_THROW_FORT, oTarget, TE_GetSpellSaveDC(OBJECT_SELF,GetSpellId(),GetLastSpellCastClass()), SAVING_THROW_TYPE_NEGATIVE))
            {
