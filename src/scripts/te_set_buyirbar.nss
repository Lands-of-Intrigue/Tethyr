...
void main()
{
    ...
    if( (GetLocalInt(oPC,TRADE_IRONBAR) >= 5 ) || ( GetItemPossessedBy(oPC,TRADE_IRONBAR) != OBJECT_INVALID) )
    {
        object oItem = GetFirstItemInInventory(oPC);
        int nCount = 0;
        while (oItem != OBJECT_INVALID)
        {
            if((GetTag(oItem) == TRADE_IRONBAR) && nCount < 5)
            {
                DestroyObject(oItem);
                nCount += 1;
            }
        ...
        nGold = FloatToInt(VALUE_IRONBAR*fMerchant);
