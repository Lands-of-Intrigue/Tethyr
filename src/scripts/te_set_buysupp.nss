...
void main()
{
    ...
    if( (GetLocalInt(oPC,TRADE_SUPPLIES) >= 1 ) || ( GetItemPossessedBy(oPC,TRADE_SUPPLIES) != OBJECT_INVALID) )
    {
        object oItem = GetItemPossessedBy(oPC,TRADE_SUPPLIES); DestroyObject(oItem);
        nGold = FloatToInt(VALUE_SUPPLIES*fMerchant);
