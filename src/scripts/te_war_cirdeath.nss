...
// End of Spell Cast Hook
    ...
    int nHD = d4(GetLevelByClass(47,OBJECT_SELF)); //Roll to see how many HD worth of creature will be killed
    ...
    {
        nHD = 4 * GetLevelByClass(47,OBJECT_SELF);//Damage is at max
    }
    ...
    while ((nHD > 0) && (bContinueLoop))
    {
        ...
        if(bContinueLoop == TRUE)
        {
            ...
            if(!MyResistSpell(OBJECT_SELF, oLowest, fDelay))
            {
                ...
                if(!MySavingThrow(SAVING_THROW_FORT, oLowest, TE_GetSpellSaveDC(OBJECT_SELF,GetSpellId(),47), SAVING_THROW_TYPE_DEATH, OBJECT_SELF, fDelay))
                {
