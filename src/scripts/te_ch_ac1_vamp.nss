...
{   //SpawnScriptDebugger();
    ...
    if ( (GetIsDay() == TRUE) &&(!GetIsAreaInterior( GetArea(OBJECT_SELF) ) ))
    {
        ApplyEffectToObject(DURATION_TYPE_INSTANT,EffectDamage((GetMaxHitPoints(OBJECT_SELF)/3),DAMAGE_TYPE_FIRE,DAMAGE_POWER_PLUS_TEN),OBJECT_SELF);
