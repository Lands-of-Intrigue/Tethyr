    ...
    while(GetIsObjectValid(oTarget))
    {
        ...
        //{
            if (spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, OBJECT_SELF) && oTarget != OBJECT_SELF)
            {
                ...
                if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay)|| GetTag(oTarget) == "te_gol_iron")
                {
                      //Roll Damage
                      ...
                      nDamage2 = GetReflexAdjustedDamage(nDamage/2, oTarget, TE_GetSpellSaveDC(OBJECT_SELF,GetSpellId(),GetLastSpellCastClass()), SAVING_THROW_TYPE_DIVINE);
