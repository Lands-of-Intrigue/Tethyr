...
void main()
{
    ...
    effect eBeamS,eBeamF;
    int nDamage = d6(nCL/2);

    if(nEssence == 1055)
    {
        eBeamS = EffectBeam(VFX_BEAM_EVIL, oPC, BODY_NODE_HAND);
        eBeamF = EffectBeam(VFX_BEAM_EVIL, oPC, BODY_NODE_HAND,TRUE);
    }
    else if(nEssence == 1056)
    {
    eBeamS = EffectBeam(VFX_BEAM_MIND, oPC, BODY_NODE_HAND);
    eBeamF = EffectBeam(VFX_BEAM_MIND, oPC, BODY_NODE_HAND,TRUE);
    }
    else if(nEssence == 1057)
    {
    eBeamS = EffectBeam(VFX_BEAM_FIRE, oPC, BODY_NODE_HAND);
    eBeamF = EffectBeam(VFX_BEAM_FIRE, oPC, BODY_NODE_HAND,TRUE);
    }
    else if(nEssence == 1058)
    {
    eBeamS = EffectBeam(VFX_BEAM_COLD, oPC, BODY_NODE_HAND);
    eBeamF = EffectBeam(VFX_BEAM_COLD, oPC, BODY_NODE_HAND,TRUE);
    }
    else if(nEssence == 1059)
    {
    eBeamS = EffectBeam(VFX_BEAM_DISINTEGRATE, oPC, BODY_NODE_HAND);
    eBeamF = EffectBeam(VFX_BEAM_DISINTEGRATE, oPC, BODY_NODE_HAND,TRUE);
    }
    else if(nEssence == 1060)
    {
    eBeamS = EffectBeam(VFX_BEAM_MIND, oPC, BODY_NODE_HAND);
    eBeamF = EffectBeam(VFX_BEAM_MIND, oPC, BODY_NODE_HAND,TRUE);
    }
    else if(nEssence == 1061)
    {
    eBeamS = EffectBeam(VFX_BEAM_BLACK, oPC, BODY_NODE_HAND);
    eBeamF = EffectBeam(VFX_BEAM_BLACK, oPC, BODY_NODE_HAND,TRUE);
    }
    else if(nEssence == 1062)
    {
    eBeamS = EffectBeam(VFX_BEAM_CHAIN, oPC, BODY_NODE_HAND);
    eBeamF = EffectBeam(VFX_BEAM_CHAIN, oPC, BODY_NODE_HAND,TRUE);
    }
    else
    {
    eBeamS = EffectBeam(VFX_BEAM_ODD, oPC, BODY_NODE_HAND);
    eBeamF = EffectBeam(VFX_BEAM_ODD, oPC, BODY_NODE_HAND,TRUE);
    }
    ...
    if(nTouch > 0)
    {
        ApplyEffectToObject(DURATION_TYPE_TEMPORARY,eBeamS,oTarget,1.0f);
    ...
    {
        ApplyEffectToObject(DURATION_TYPE_TEMPORARY,eBeamF,oTarget,1.0f);
    }
}
