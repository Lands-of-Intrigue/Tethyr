...
void main()
{
            ...
            if(GetIsObjectValid(oMyWeapon))
            {
                ...
                if (nDuration > 0)
                {
                    if(GetHasFeat(1481,GetItemPossessor(oMyWeapon)))//alchemy proficiency
                         {
                         nDuration = 20;
                         SendMessageToPC(GetItemPossessor(oMyWeapon), "You apply alchemist's fire more effectively due to your knowledge of the substance.");
                         }
    ...
    {
        DoGrenade(d8(1),d4(1)+1, VFX_IMP_FLAME_M, VFX_FNF_FIREBALL,DAMAGE_TYPE_FIRE,RADIUS_SIZE_LARGE, OBJECT_TYPE_CREATURE | OBJECT_TYPE_DOOR | OBJECT_TYPE_PLACEABLE);
    }
}
