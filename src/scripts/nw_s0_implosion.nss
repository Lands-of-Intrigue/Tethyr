    ...
    while (GetIsObjectValid(oTarget))
    {
        if (oTarget != OBJECT_SELF && spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
        {
           ...
           if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay))
           {
                ...
                {
                //Make Reflex save
                if(!MySavingThrow(SAVING_THROW_FORT, oTarget, TE_GetSpellSaveDC(OBJECT_SELF,GetSpellId(),GetLastSpellCastClass())+3, SAVING_THROW_TYPE_DEATH, OBJECT_SELF, fDelay))
                {
