...
void main()
{
    ...
    if( (GetLocalInt(oPC,TRADE_WAGON) >= 1 ) || ( GetItemPossessedBy(oPC,TRADE_WAGON) != OBJECT_INVALID) )
    {
        object oItem = GetItemPossessedBy(oPC,TRADE_WAGON); DestroyObject(oItem);
        nGold = FloatToInt(VALUE_WAGON*fMerchant);
