...
//:://////////////////////////////////////////////
/*
Patch 1.70, complete rewrite by Shadooow
...
void main()
{
    ...
    if(bHarmful)
    {
        ...
        if(GetIsPC(oCaster))
            SetLocalInt(OBJECT_SELF, "COMBATANT_"+ObjectToString(oCaster), GetTimeCumulative());
        else if(GetAssociateType(oCaster)!=ASSOCIATE_TYPE_NONE)
            SetLocalInt(OBJECT_SELF, "COMBATANT_"+ObjectToString(GetMaster(oCaster)), GetTimeCumulative());
    ...
    if(GetSpawnInCondition(NW_FLAG_SPELL_CAST_AT_EVENT))
    {
        SetLocalInt(OBJECT_SELF, "USERD_LASTSPELL", nSpell);
