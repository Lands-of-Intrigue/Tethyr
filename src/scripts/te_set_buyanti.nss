...
void main()
{
    ...
    if( (GetLocalInt(oPC,TRADE_ANTIDOTE) >= 5 ) || ( GetItemPossessedBy(oPC,TRADE_ANTIDOTE) != OBJECT_INVALID) )
    {
        object oItem = GetFirstItemInInventory(oPC);
        int nCount = 0;
        while (oItem != OBJECT_INVALID)
        {
            if((GetTag(oItem) == TRADE_ANTIDOTE) && nCount < 5)
            {
                DestroyObject(oItem);
                nCount += 1;
            }
        ...
        nGold = FloatToInt(VALUE_ANTIDOTE*fMerchant);
