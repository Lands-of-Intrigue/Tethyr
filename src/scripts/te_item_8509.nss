...
void main()
{
    ...
    switch (nEvent)
    {
        ...
        case X2_ITEM_EVENT_ACTIVATE:
            ...
            oDoor = GetItemActivatedTarget();

            if(GetObjectType(oDoor) == OBJECT_TYPE_DOOR)
            {
                sOwner    = GetLocalString(oDoor,"sOwner");
                sSetOwner = GetCampaignString("Settlement",sOwner+"_sOwner");
                if (sOwner == "")    {sOwner = "Blank";}
                if (sSetOwner == "") {sOwner = "Blank";}
                if(GetLocalString(oItem,"Settlement") == sOwner || GetLocalString(oItem,"Settlement") == sSetOwner)
                {
                    AssignCommand(oDoor,ActionStartConversation(oPC,"te_door",TRUE,FALSE));
                }
                else
                {
                    SendMessageToPC(oPC,"You are unable to do anything with this door.");
                }
            }
            else
            {
                SendMessageToPC(oPC,"You may only target doors that you own with this ability.");
            }
