...
void main()
{
    ...
    string sUnique = GetLocalString(oDoor,"sUnique");
    int nState     = GetCampaignInt("Housing",sUnique+"Stat");



    if(nState >= 2 && GetIsReactionTypeFriendly(oDoor,OBJECT_SELF) == TRUE)
    {
        ActionOpenDoor(OBJECT_SELF); //Open the Door.
        DelayCommand(12.0, ActionAutoCloseAndLock(OBJECT_SELF)); //Delay 6 seconds before auto closing and locking the door.
    }
    else
    {
