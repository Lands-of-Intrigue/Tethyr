...
// Removes any visual effects (including polymorph and invisibility) from the selected character
void GestaltClearFX(object oActor);

// Removes any effects that might interfere with oActor's movement (sleep, paralyzation etc)
...
// Copies a creature or item
// IMPORTANT NOTE - as of NWN v1.32 there is STILL a bug in the game which stops you from locating a copied object by its tag
// As a result this function won't work properly. You should use object oClone = CopyObject(...); to clone characters instead
...
//--j's new unique name code: 5/2006
    SetLocalInt(GetModule(),sName,iCancel); //-- "window" = 1 (2,3,etc)
    sName = sName + "_" + IntToString(iCancel);//-- window_1 (_2,_3,etc)
//--------------------------------------------
    SetLocalString(GetModule(),"cutscene",sName); //--"window_1"
//--removed 5/2006    SetLocalInt(GetModule(),sName + "time",iCancel);

    while (GetIsObjectValid(oParty))
        {
        SetCutsceneMode(oParty,TRUE);
        SetLocalString(oParty,"cutscene",sName);
//--removed 5/2006        SetLocalInt(oParty,sName + "time",iCancel);
...
void GestaltRegisterActor(string sName, object oActor, string sActor = "")
{
    ...
    string sCancel = ""; //--removed 5/2006  IntToString(GetLocalInt(GetModule(),sName + "time"));

    GestaltResetSpeed(oActor);
    SetCutsceneCameraMoveRate(oActor,1.0);

    if (GetLocalInt(GetModule(),sName + sCancel + sActor + "registered") > 0)
        { return; }
    ...
    int iActors = GetLocalInt(GetModule(),sName + sCancel + "actorsregistered") + 1;
    SetLocalObject(GetModule(),sName + sCancel + "actor" + IntToString(iActors),oActor);
    SetLocalInt(GetModule(),sName + sCancel + "actorsregistered",iActors);
    SetLocalInt(GetModule(),sName + sCancel + sActor + "registered",1);
}
...
void GestaltClearActorSpeeds(string sName)
{
    ...
    int iCount = 1;
    int iActors = GetLocalInt(GetModule(),sName + sCancel + "actorsmodified");
    object oActor;

    while (iCount <= iActors)
        {
        oActor = GetLocalObject(GetModule(),sName + sCancel + "actorspeedmodified" + IntToString(iCount));
        GestaltResetSpeed(oActor);
        iCount++;
        }
}
...
void GestaltClearActors(string sName)
{
    string sCancel = "";  //--removed 5/2006  IntToString(GetLocalInt(GetModule(),sName + "time"));

    int iCount = 1;
    int iActors = GetLocalInt(GetModule(),sName + sCancel + "actorsregistered");
    object oActor = GetLocalObject(GetModule(),sName + sCancel + "actor" + IntToString(iCount));
    object oWP;
    ...
    while (iCount <= iActors)
        ...
        oWP = GetWaypointByTag(sName + GetTag(oActor));

        if (GetIsObjectValid(oWP) && GetIsObjectValid(oActor) && GetDistanceBetween(oWP,oActor) > 1.0)
            {
        ...
        iCount++;
        oActor = GetLocalObject(GetModule(),sName + sCancel + "actor" + IntToString(iCount));
        }
}



void GestaltDoStopCutscene(string sName, int iCancel, object oPC, string sDestination = "", int bMode = TRUE, int bCamera = TRUE, int bClear = TRUE, int bClearFX = TRUE, int bResetSpeed = TRUE, int bClearActors = TRUE, int iParty = 0)
{
    // Check cutscene hasn't been stopped already
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    ...
    if (sName != "")
        {
//--removed 5/2006        SetLocalInt(GetModule(),sName,iCancel);
//--replaced with
        SetLocalInt(GetModule(),sName,TRUE);
        }
//--bloodsong adds deletion of module string
    ...
    if (bClearActors)           { GestaltClearActors(sName); }

    GestaltClearActorSpeeds(sName);
...
//--removed 5/2006        DeleteLocalInt(oParty,sName + "time");
...
//--removed 5/2006    int iCancel = GetLocalInt(oPC,sName + "time");

    DelayCommand(fDelay,GestaltDoStopCutscene(sName, 0,oPC,sDestination,bMode,bCamera,bClear,bClearFX,bResetSpeed,bClearActors,iParty));
}
...
{
  if(oAssociate == OBJECT_INVALID) {  return;  SendMessageToPC(GetFirstPC(), "GestaltDOClearAssociate Aborted");  }
...
void GestaltDoClearAssociates(string sName, int iCancel, object oPC, int iAssociates = 63, int iMethod = 0, object oDestination = OBJECT_INVALID, string sDestination = "")
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    while (GetIsObjectValid(oParty))
        {
        DelayCommand(fDelay,GestaltDoClearAssociates(sName, 0,oParty,iAssociates,iMethod,oDestination,sDestination));
...
void GestaltDoReturnAssociates(string sName, int iCancel, object oPC, int iAssociates, int iMethod, object oDestination, string sDestination)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    while (GetIsObjectValid(oParty))
        {
        DelayCommand(fDelay,GestaltDoReturnAssociates(sName, 0,oParty,iAssociates,iMethod,oDestination,sDestination));
...
void GestaltDoSetSpeed(string sName, int iCancel, object oActor, string sActor, float fTime, float fDistance, int iRun, float fStops = 0.0)
{
    if (fTime == 0.0)                               { return; }
...
//--replaced with
    ...
    if (!GetIsPC(oActor))
        ...
        string sRecord = sName + IntToString(iCancel);
        int iCount = GetLocalInt(GetModule(),sRecord + "actorsmodified") + 1;
        SetLocalInt(GetModule(),sRecord + "actorsmodified",iCount);
        SetLocalObject(GetModule(),sRecord + "actorspeedmodified" + IntToString(iCount),oActor);
...
{
    string sName = GetLocalString(GetModule(),"cutscene");
//removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");

    DelayCommand(fDelay,GestaltDoSetSpeed(sName, 0,OBJECT_INVALID,sActor,fTime,fDistance,iRun));
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoSetSpeed(sName, 0,oActor,"",fTime,fDistance,iRun));
}



void GestaltDoInvisibility(string sName, int iCancel, object oActor, string sActor, float fTime)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoInvisibility(sName, 0,oActor,sActor,fTime));
}



void GestaltDoMove(string sName, int iCancel, object oActor, string sActor, object oDestination, int iRun, float fRange, float fTime, string sDestination, int bTowards)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
    ...
    if (fTime > 0.0)
        { AssignCommand(oActor,ActionDoCommand(GestaltDoSetSpeed(sName, 0,oActor,"",fTime,GetDistanceBetween(oActor,oDestination),iRun))); }
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoMove(sName, 0,OBJECT_INVALID,sActor,oDestination,iRun,fRange,fTime,sDestination,bTowards));
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoMove(sName, 0,oActor,"",oDestination,iRun,fRange,fTime,sDestination,bTowards));
    if (!GetIsPC(oActor))       { GestaltRegisterActor(sName,oActor); }
}



void GestaltDoJump(string sName, int iCancel, object oActor, string sActor, object oTarget, string sTarget, int bAction = FALSE)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoJump(sName, 0,OBJECT_INVALID,sActor,oTarget,sTarget,TRUE));
    GestaltRegisterActor(sName,OBJECT_INVALID,sActor);
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoJump(sName, 0,OBJECT_INVALID,sActor,oTarget,sTarget));
    GestaltRegisterActor(sName,OBJECT_INVALID,sActor);
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoJump(sName, 0,oActor,"",oTarget,sTarget,TRUE));
    if (!GetIsPC(oActor))       { GestaltRegisterActor(sName,oActor); }
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoJump(sName, 0,oActor,"",oTarget,sTarget));
    if (!GetIsPC(oActor))       { GestaltRegisterActor(sName,oActor); }
}



void GestaltDoAnimate(string sName, int iCancel, object oActor, string sActor, int iAnim, float fDuration = 0.0, float fSpeed = 1.0, int bAction = FALSE)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoAnimate(sName, 0,OBJECT_INVALID,sActor,iAnim,fDuration,fSpeed,TRUE));
    GestaltRegisterActor(sName,OBJECT_INVALID,sActor);
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoAnimate(sName, 0,OBJECT_INVALID,sActor,iAnim,fDuration,fSpeed));
    GestaltRegisterActor(sName,OBJECT_INVALID,sActor);
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoAnimate(sName, 0,oActor,"",iAnim,fDuration,fSpeed,TRUE));
    if (!GetIsPC(oActor))       { GestaltRegisterActor(sName,oActor); }
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoAnimate(sName, 0,oActor,"",iAnim,fDuration,fSpeed));
    if (!GetIsPC(oActor))       { GestaltRegisterActor(sName,oActor); }
}



void GestaltDoSpeak(string sName, int iCancel, object oActor, string sActor, string sLine, int iAnimation = ANIMATION_NONE, float fDuration = 0.0, float fSpeed = 1.0, int bAction = FALSE)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoSpeak(sName, 0,OBJECT_INVALID,sActor,sLine,iAnimation,fDuration,fSpeed,TRUE));
    GestaltRegisterActor(sName,OBJECT_INVALID,sActor);
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoSpeak(sName, 0,OBJECT_INVALID,sActor,sLine,iAnimation,fDuration,fSpeed));
    GestaltRegisterActor(sName,OBJECT_INVALID,sActor);
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoSpeak(sName, 0,oActor,"",sLine,iAnimation,fDuration,fSpeed,TRUE));
    if (!GetIsPC(oActor))       { GestaltRegisterActor(sName,oActor); }
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoSpeak(sName, 0,oActor,"",sLine,iAnimation,fDuration,fSpeed));
    if (!GetIsPC(oActor))       { GestaltRegisterActor(sName,oActor); }
}



void GestaltDoConversation(string sName, int iCancel, object oActor, string sActor, object oTarget, string sConv = "", string sTarget = "", int bGreet = TRUE)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoConversation(sName, 0,oActor,"",oTarget,sConv,sTarget,bGreet));
    if (!GetIsPC(oActor))       { GestaltRegisterActor(sName,oActor); }
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoConversation(sName, 0,OBJECT_INVALID,sActor,oTarget,sConv,sTarget,bGreet));
    GestaltRegisterActor(sName,OBJECT_INVALID,sActor);
}



void GestaltDoFace(string sName, int iCancel, object oActor, string sActor, object oTarget, int iFace, float fFace, int bAction)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoFace(sName, 0,OBJECT_INVALID,sActor,oTarget,iFace,fFace,TRUE));
    GestaltRegisterActor(sName,OBJECT_INVALID,sActor);
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoFace(sName, 0,OBJECT_INVALID,sActor,oTarget,iFace,fFace,FALSE));
    GestaltRegisterActor(sName,OBJECT_INVALID,sActor);
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoFace(sName, 0,oActor,"",oTarget,iFace,fFace,TRUE));
    if (!GetIsPC(oActor))       { GestaltRegisterActor(sName,oActor); }
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoFace(sName, 0,oActor,"",oTarget,iFace,fFace,FALSE));
    if (!GetIsPC(oActor))       { GestaltRegisterActor(sName,oActor); }
}



void GestaltDoEquip(string sName, int iCancel, object oActor, string sActor, int iSlot, object oItem, string sItem)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoEquip(sName, 0,OBJECT_INVALID,sActor,iSlot,oItem,sItem));
    GestaltRegisterActor(sName,OBJECT_INVALID,sActor);
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoEquip(sName, 0,oActor,"",iSlot,oItem,sItem));
    if (!GetIsPC(oActor))       { GestaltRegisterActor(sName,oActor); }
}



void GestaltDoUnequip(string sName, int iCancel, object oActor, string sActor, int iSlot, object oItem, string sItem)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoUnequip(sName, 0,OBJECT_INVALID,sActor,iSlot,oItem,sItem));
    GestaltRegisterActor(sName,OBJECT_INVALID,sActor);
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoUnequip(sName, 0,oActor,"",iSlot,oItem,sItem));
    if (!GetIsPC(oActor))       { GestaltRegisterActor(sName,oActor); }
}



void GestaltDoAttack(string sName, int iCancel, object oActor, string sActor, object oTarget, int bPassive, string sTarget)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoAttack(sName, 0,OBJECT_INVALID,sActor,oTarget,bPassive,sTarget));
    GestaltRegisterActor(sName,OBJECT_INVALID,sActor);
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoAttack(sName, 0,oActor,"",oTarget,bPassive,sTarget));
    if (!GetIsPC(oActor))       { GestaltRegisterActor(sName,oActor); }
}



void GestaltDoSpellCast(string sName, int iCancel, object oActor, string sActor, object oTarget, int iSpell, int bFake = FALSE, int iPath = PROJECTILE_PATH_TYPE_DEFAULT, string sTarget = "", int bCheat = TRUE, int bInstant = FALSE, int iLevel = 0, int iMeta = METAMAGIC_NONE)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoSpellCast(sName, 0,oActor,"",oTarget,iSpell,bFake,iPath,sTarget,bCheat,bInstant,iLevel,iMeta));
    if (!GetIsPC(oActor))       { GestaltRegisterActor(sName,oActor); }
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoSpellCast(sName, 0,OBJECT_INVALID,sActor,oTarget,iSpell,bFake,iPath,sTarget,bCheat,bInstant,iLevel,iMeta));
    GestaltRegisterActor(sName,OBJECT_INVALID,sActor);
}



void GestaltDoEffect(string sName, int iCancel, object oActor, string sActor, object oTarget, string sTarget, effect eFect, int iDuration = PERMANENT, float fDuration = 0.0, int bAction = FALSE)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoEffect(sName, 0,OBJECT_INVALID,"",oTarget,sTarget,eFect,iDuration,fDuration));
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoEffect(sName, 0,oActor,"",oTarget,sTarget,eFect,iDuration,fDuration,TRUE));
    if (!GetIsPC(oActor))       { GestaltRegisterActor(sName,oActor); }
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoEffect(sName, 0,OBJECT_INVALID,sActor,oTarget,sTarget,eFect,iDuration,fDuration,TRUE));
    GestaltRegisterActor(sName,OBJECT_INVALID,sActor);
}
...
void GestaltDoCreate(string sName, int iCancel, object oActor, string sActor, object oTarget, string sTarget, int iType, string sRef, string sTag, int iAnim, int iStack, int bCreateOn, int bAction = FALSE)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoCreate(sName, 0,oActor,"",oTarget,sTarget,iType,sRef,sTag,iAnim,iStack,bCreateOn,TRUE));
    if (!GetIsPC(oActor))       { GestaltRegisterActor(sName,oActor); }
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoCreate(sName, 0,OBJECT_INVALID,sActor,oTarget,sTarget,iType,sRef,sTag,iAnim,iStack,bCreateOn,TRUE));
    GestaltRegisterActor(sName,OBJECT_INVALID,sActor);
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoCreate(sName, 0,OBJECT_INVALID,"",oTarget,sTarget,iType,sRef,sTag,iAnim,iStack,bCreateOn));
}



void GestaltDoCopy(string sName, int iCancel, object oSource, object oTarget, string sTarget, string sTag, int bCreateOn)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoCopy(sName, 0,oSource,oTarget,sTarget,sTag,bCreateOn));
}



void GestaltDoClone(string sName, int iCancel, object oPC, object oTarget, string sTarget, string sTag, int bInvisible)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
    ...
    if(GetIsPC(oPC))
    {//--6/2006: bloodsong adds this to make pc clones NOT hostile
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoClone(sName, 0,oPC,oTarget,sTarget,sTag,bInvisible));
}



void GestaltDoPickUp(string sName, int iCancel, object oActor, string sActor, object oItem, string sItem)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoPickUp(sName, 0,OBJECT_INVALID,sActor,oItem,sItem));
    GestaltRegisterActor(sName,OBJECT_INVALID,sActor);
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoPickUp(sName, 0,oActor,"",oItem,sItem));
    if (!GetIsPC(oActor))       { GestaltRegisterActor(sName,oActor); }
}



void GestaltDoSit(string sName, int iCancel, object oActor, string sActor, object oChair, string sChair)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoSit(sName, 0,OBJECT_INVALID,sActor,oChair,sChair));
    GestaltRegisterActor(sName,OBJECT_INVALID,sActor);
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoSit(sName, 0,oActor,"",oChair,sChair));
    if (!GetIsPC(oActor))       { GestaltRegisterActor(sName,oActor); }
}



void GestaltDoPlaySound(string sName, int iCancel, object oActor, string sActor, string sSound, int bAction)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoPlaySound(sName, 0,oActor,sActor,sSound,FALSE));

    if (sActor != "")           { GestaltRegisterActor(sName,OBJECT_INVALID,sActor); }
    else if (!GetIsPC(oActor))  { GestaltRegisterActor(sName,oActor); }
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoPlaySound(sName, 0,oActor,sActor,sSound,TRUE));
...
void GestaltDoSoundObject(string sName, int iCancel, object oSound, int bOn, float fDuration, int iVolume, object oPosition)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
    ...
    if (fDuration > 0.0)                { DelayCommand(fDuration,GestaltDoSoundObject(sName, 0,oSound,!bOn,0.0,128,OBJECT_INVALID)); }
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoSoundObject(sName, 0,oSound,bOn,fDuration,iVolume,oPosition));
}



void GestaltDoAmbientSound(string sName, int iCancel, object oArea, int bOn, float fDuration, int iVolume)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
    ...
    if (fDuration > 0.0)                { DelayCommand(fDuration,GestaltDoAmbientSound(sName, 0,oArea,!bOn,0.0,128)); }
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoAmbientSound(sName, 0,oArea,bOn,fDuration,iVolume));
}



void GestaltDoMusic(string sName, int iCancel, object oArea, int bOn = TRUE, int iTrack = TRACK_CURRENT, float fDuration = 0.0)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
    ...
    if (fDuration > 0.0)                { DelayCommand(fDuration,GestaltDoMusic(sName, 0,oArea,!bOn,iTrack)); }
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoMusic(sName, 0,oArea,bOn,iTrack,fDuration));
}



void GestaltDoDoor(string sName, int iCancel, object oActor, string sActor, object oDoor, int bLock, int bOpen)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoDoor(sName, 0,oActor,"",oDoor,bLock,FALSE));
    if (!GetIsPC(oActor))       { GestaltRegisterActor(sName,oActor); }
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoDoor(sName, 0,oActor,"",oDoor,bUnlock,TRUE));
    if (!GetIsPC(oActor))       { GestaltRegisterActor(sName,oActor); }
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoDoor(sName, 0,OBJECT_INVALID,sActor,oDoor,bLock,TRUE));
    GestaltRegisterActor(sName,OBJECT_INVALID,sActor);
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoDoor(sName, 0,OBJECT_INVALID,sActor,oDoor,bUnlock,TRUE));
    GestaltRegisterActor(sName,OBJECT_INVALID,sActor);
}



void GestaltDoQuest(string sName, int iCancel, object oPC, string sQuest, int iState, int iXP = 0, int iParty = 0, int bRewardAll = TRUE, int bOverride = FALSE)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoQuest(sName, 0,oPC,sQuest,iState,iXP,iParty,bRewardAll,bOverride));
}



void GestaltDoWait(string sName, int iCancel, object oActor, string sActor, float fPause)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoWait(sName, 0,OBJECT_INVALID,sActor,fPause));
    GestaltRegisterActor(sName,OBJECT_INVALID,sActor);
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoWait(sName, 0,oActor,"",fPause));
    if (!GetIsPC(oActor))       { GestaltRegisterActor(sName,oActor); }
}



void GestaltDoClear(string sName, int iCancel, object oActor, string sActor)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoClear(sName, 0,oActor,sActor));
}



void GestaltDoFloatingText(string sName, int iCancel, object oActor, string sMessage, int bFaction = TRUE)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoFloatingText(sName, 0,oActor,sMessage,bFaction));
}



void GestaltDoExecute(string sName, int iCancel, object oTarget, string sScript, string sTarget)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoExecute(sName, 0,oTarget,sScript,sTarget));
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,AssignCommand(oActor,ActionDoCommand(GestaltDoExecute(sName, 0,oTarget,sScript,sTarget))));
    if (!GetIsPC(oActor))       { GestaltRegisterActor(sName,oActor); }
}



void GestaltDoDestroy(string sName, int iCancel, object oActor, string sActor, object oTarget, string sTarget, int bAction)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoDestroy(sName, 0,oActor,"",oTarget,sTarget,TRUE));
    if (!GetIsPC(oActor))       { GestaltRegisterActor(sName,oActor); }
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoDestroy(sName, 0,OBJECT_INVALID,sActor,oTarget,sTarget,TRUE));
    GestaltRegisterActor(sName,OBJECT_INVALID,sActor);
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoDestroy(sName, 0,OBJECT_INVALID,"",oTarget,sTarget,FALSE));
}
...
void GestaltDoCameraMode(string sName, int iCancel, object oPC, int iMode)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoCameraMode(sName, 0,oPC,iMode));
}



void GestaltDoCameraFacing(string sName, int iCancel, float fDirection, float fRange, float fPitch, object oPC, int iTransition)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoCameraFacing(sName, 0,fDirection,fRange,fPitch,oPC,iTransition));
}
...
void GestaltDoCameraSetup(string sName, int iCancel, object oPC, float fDirection, float fRange, float fPitch, float fHeight)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    if (fDelay == 0.0)  { GestaltDoCameraSetup(sName, 0,oPC,fDirection,fRange,fPitch,fHeight); }
    else                { DelayCommand(fDelay,GestaltDoCameraSetup(sName, 0,oPC,fDirection,fRange,fPitch,fHeight)); }
}
...
void GestaltDoFadeOut(string sName, int iCancel, object oPC, float fSpeed, int iParty)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
void GestaltDoFadeIn(string sName, int iCancel, object oPC, float fSpeed, int iParty)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
void GestaltDoBlack(string sName, int iCancel, object oPC, int iParty)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
void GestaltDoStopFade(string sName, int iCancel, object oPC, int iParty)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
...
void GestaltDoFade(string sName, int iCancel, object oPC, int iFade, float fSpeed, float fDuration, int iParty)
{
//--removed 5/2006    if (GetLocalInt(GetModule(),sName) >= iCancel && iCancel > 0)
//--replaced with
    if (GetLocalInt(GetModule(),sName))
    ...
    if (iFade == FADE_IN)
        {
        if (fDuration > 0.0)
            { GestaltDoBlack(sName, 0,oPC,iParty); }
        DelayCommand(fDuration,GestaltDoFadeIn(sName, 0,oPC,fSpeed,iParty));
        }

    else if (iFade == FADE_OUT)
        {
        GestaltDoFadeOut(sName, 0,oPC,fSpeed,iParty);
        if (fDuration > 0.0)
            { DelayCommand(fDuration,GestaltDoStopFade(sName, 0,oPC,iParty)); }
        }
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,AssignCommand(oActor,ActionDoCommand(GestaltDoFade(sName, 0,oPC,iFade,fSpeed,fDuration,iParty))));
    if (!GetIsPC(oActor))       { GestaltRegisterActor(sName,oActor); }
}
...
//--removed 5/2006    int iCancel = GetLocalInt(GetModule(),sName + "time");
    ...
    DelayCommand(fDelay,GestaltDoFade(sName, 0,oPC,iFade,fSpeed,fDuration,iParty));
}
