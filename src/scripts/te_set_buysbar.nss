...
void main()
{
    ...
    if( (GetLocalInt(oPC,TRADE_SILVERBARK) >= 5 ) || ( GetItemPossessedBy(oPC,TRADE_SILVERBARK) != OBJECT_INVALID) )
    {
        object oItem = GetFirstItemInInventory(oPC);
        int nCount = 0;
        while (oItem != OBJECT_INVALID)
        {
            if((GetTag(oItem) == TRADE_SILVERBARK) && nCount < 5)
            {
                DestroyObject(oItem);
                nCount += 1;
            }
        ...
        nGold = FloatToInt(VALUE_SILVERBARK*fMerchant);
