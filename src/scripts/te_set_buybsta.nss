...
void main()
{
    ...
    if( (GetLocalInt(oPC,TRADE_BLOODSTAUNCH) >= 5 ) || ( GetItemPossessedBy(oPC,TRADE_BLOODSTAUNCH) != OBJECT_INVALID) )
    {
        object oItem = GetFirstItemInInventory(oPC);
        int nCount = 0;
        while (oItem != OBJECT_INVALID)
        {
            if((GetTag(oItem) == TRADE_BLOODSTAUNCH) && nCount < 5)
            {
                DestroyObject(oItem);
                nCount += 1;
            }
        ...
        nGold = FloatToInt(VALUE_BLOODSTAUNCH*fMerchant);
