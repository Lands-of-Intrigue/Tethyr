...
object oSwap = GetObjectByTag("ITEM_SWAP");
string CRP_DESTROYED_ITEM = "";
int CRP_ALT_DAMAGE = FALSE;
int CRP_CONTAINER_HAS_BEEN_DESTROYED = 0;
const int CRP_CHANCE_OF_NO_MONSTERTREASURE = 20;

//CRP END CODE ADDITION
...
//CRP CODE ADDITION
int crp_GetIsItemDestroyed(object oItem, int nAltDamage)
{
    if(GetPlotFlag(oItem)) return FALSE;
    int nBaseType = GetBaseItemType(oItem);

    if(CRP_DEBUG)
       SendMessageToPC(GetFirstPC(),GetTag(oItem)+ " " +IntToString(nBaseType));

    //damage chances
    int nAm, nCl, nBl, nBt, nBk, nAr, nLB, nMSm, nMLg, nMMd, nMTh;
    int nPt, nScr, nSB, nTr, nTrp, nHlr;

    if(nAltDamage){
        nPt=30; nCl=80; nBl=50; nBt=60; nScr=99; nMSm=60; nMLg=60; nMMd=60;
        nMTh=60; nAr=80; nLB=40; nSB=40; nBk=85; nTr=99; nTrp=40; nHlr=70;
        CRP_DESTROYED_ITEM = "crpi_ashes";}
    else{
        nPt=90; nAm=40; nAr=60; nLB=20; nSB=20; nMSm=40; nMLg=40;
        nMMd=40; nMTh=40;
        CRP_DESTROYED_ITEM = "crpi_broken";}
    ...
    switch(nBaseType)
    {
        case BASE_ITEM_AMULET:
            if(nSaveRoll < nAm) return TRUE;
        case BASE_ITEM_CLOAK:
            if(nSaveRoll < nCl) return TRUE;
        case BASE_ITEM_BELT:
            if(nSaveRoll < nBl) return TRUE;
        case BASE_ITEM_BOOTS:
            if(nSaveRoll < nBt) return TRUE;
        case BASE_ITEM_BOOK:
            if(nSaveRoll < nBk) return TRUE;
        case BASE_ITEM_ARROW:
        case BASE_ITEM_BOLT:
            if(nSaveRoll < nAr){
                if(nAltDamage) return TRUE;
                else {
                    CRP_DESTROYED_ITEM = "crpi_brkn_wood";
                    return TRUE;
                }
            }
        ...
        case BASE_ITEM_MISCLARGE:
            if(nSaveRoll < nMLg) return TRUE;
        case BASE_ITEM_MISCMEDIUM:
            if(nSaveRoll < nMMd) return TRUE;
        case BASE_ITEM_MISCSMALL:
            if(nSaveRoll < nMSm) return TRUE;
        case BASE_ITEM_MISCTHIN:
            if(nSaveRoll < nMTh) return TRUE;
        ...
        case 75:  //Scroll
            if(nSaveRoll < nScr) return TRUE;
        case BASE_ITEM_SHORTBOW:
            if(nSaveRoll < nSB) return TRUE;
        case BASE_ITEM_TORCH:
            if(nSaveRoll < nTr) return TRUE;
        case BASE_ITEM_TRAPKIT:
            if(nSaveRoll < nTrp) return TRUE;
        case BASE_ITEM_HEALERSKIT:
            if(nSaveRoll < nHlr) return TRUE;
        default: return FALSE;
    }
    return FALSE;
}
int crp_BreakFragileItems(int nNoAutoGen=0)
{
    ...
    if(bDamage >= 7) CRP_ALT_DAMAGE = TRUE;

    if(CRP_DEBUG) SendMessageToPC(GetFirstPC(), "Alt Damage: " +
                  IntToString(CRP_ALT_DAMAGE));
    ...
    if(GetIsObjectValid(oItem))
    {
        ...
        while(GetIsObjectValid(oItem))
        {
            ...
            {
                nCnt++;
                SetLocalString(OBJECT_SELF,"BKN"+IntToString(nCnt),CRP_DESTROYED_ITEM);
                DestroyObject(oItem);
            }
        ...
        while(nCnt >= 1)
        {
            sBkn = GetLocalString(OBJECT_SELF, "BKN"+IntToString(nCnt));
            CreateItemOnObject(sBkn);
            nCnt--;
        }
    }
    return nNoAutoGen;
...
// *
    ...
    void CreateBook(object oTarget)
    {
        ...
        {
            object oTemp = CreateItemOnObject(sRes, oSwap);
            if(crp_GetIsItemDestroyed(oTemp, CRP_ALT_DAMAGE))
                dbCreateItemOnObject(CRP_DESTROYED_ITEM, oTarget);
            else
    ...
    void CreateGold(object oTarget, object oAdventurer, int nTreasureType, int nModifier = 0)
    {
        ...
        if(CRP_USE_COINS)
        {
            nTreasureType = nTreasureType * nTreasureType;

            if(!GetIsPC(oAdventurer))
            {
                nHD = FloatToInt(GetChallengeRating(oTarget));
                if(nHD < 1) nHD = 1;
            }
            //chance of having a particular coin type
            int CP, SP, GP, PL, nMod=0;
            //number of 4 sided dice to roll to determine coin number
            //if 0, the coin amount is exactly 1 coin
            int CP_AMT, SP_AMT, GP_AMT, PL_AMT;

            switch(nHD)
            {
                case 1: case 2:
                CP = 80;    SP = 20;    GP = 5;     PL = 0;
                CP_AMT = 2; SP_AMT = 1; GP_AMT = 0; PL_AMT = 0;
                break;
                case 3: case 4: case 5:
                CP = 40; SP = 50; GP = 20; PL = 1;
                CP_AMT = 1; SP_AMT = 2; GP_AMT = 1; PL_AMT = 0;
                break;
                case 6: case 7: case 8: case 9:
                CP = 0; SP = 30; GP = 80; PL = 10;
                CP_AMT = 0; SP_AMT = 5; GP_AMT = 5; PL_AMT = 1;
                break;
                case 10: case 11: case 12: case 13: case 14:
                CP = 0; SP = 20; GP = 100; PL = 25;
                CP_AMT = 0; SP_AMT = 3; GP_AMT = 10; PL_AMT = 2;
                break;
                default:
                CP = 0; SP = 10; GP = 100; PL = 25;
                CP_AMT = 0; SP_AMT = 1; GP_AMT = 15; PL_AMT = 3;
            }
            if(d100() <= PL){
                nMod = 0;
                if(PL_AMT == 0) PL_AMT = 1;
                else PL_AMT = d4(PL_AMT);
                GiveCoinsTo(oTarget, PL_AMT * nTreasureType, COIN_PLATINUM); }
            else nMod = nMod + 10;
            if(d100() <= GP + nMod){
                nMod = 0;
                if(GP_AMT == 0) GP_AMT = 1;
                else GP_AMT = d4(GP_AMT);
                GiveCoinsTo(oTarget, GP_AMT * nTreasureType, COIN_GOLD);}
            else nMod = nMod + 10;
            if(d100() <= SP + nMod){
                nMod = 0;
                if(SP_AMT == 0) SP_AMT = 1;
                else SP_AMT = d4(SP_AMT);
                GiveCoinsTo(oTarget, SP_AMT * nTreasureType, COIN_SILVER);}
            else nMod = nMod + 10;
            if(d100() <= CP + nMod){
                if(CP_AMT == 0) CP_AMT = 1;
                else CP_AMT = d4(CP_AMT);
                GiveCoinsTo(oTarget, CP_AMT * nTreasureType, COIN_COPPER);}
        }
        else //BIOWARE ORIGINAL CODE
        {
            if (GetRange(1, nHD)) nAmount = d10();
    ...
    void CreateGem(object oTarget, object oAdventurer, int nTreasureType, int nModifier = 0)
    {
        ...
        if(CRP_USE_GEMS)
        {
            string sGem = "crpi_o_malachite";
            if (GetRange(1, nHD))
            {
                int nRandom = Random(6) + 1;
                switch (nRandom)
                {
                    case 1: sGem = "crpi_o_malachite";      break;
                    case 2: sGem = "crpi_o_obsidian";       break;
                    case 3: sGem = "crpi_o_quartz";         break;
                    case 4: sGem = "crpi_o_rhodochro";      break;
                    case 5: sGem = "crpi_o_tigereye";       break;
                    case 6: sGem = "crpi_o_turquoise";      break;
                }
            }
            else if (GetRange(2, nHD)) // 50 GP Avg; 100 gp Max
            {
                int nRandom = Random(15) + 1;
                switch (nRandom)
                {
                    case 1: sGem = "crpi_s_bloodston";      break;
                    case 2: sGem = "crpi_s_carnelian";      break;
                    case 3: sGem = "crpi_s_chalcedon";      break;
                    case 4: sGem = "crpi_s_chrysopra";      break;
                    case 5: sGem = "crpi_s_citrine";        break;
                    case 6: sGem = "crpi_s_jasper";         break;
                    case 7: sGem = "crpi_s_moonstone";      break;
                    case 8: sGem = "crpi_s_onyx";           break;
                    case 9: sGem = "crpi_s_rockcryst";      break;
                    case 10: sGem = "crpi_s_sardonyx";      break;
                    case 11: sGem = "crpi_s_starrose";      break;
                    case 12: sGem = "crpi_s_zircon";        break;
                    case 13: sGem = "crpi_f_alexandri";     break;
                    case 14: sGem = "crpi_f_amber";         break;
                    case 15: sGem = "crpi_o_malachite";     break;
                }
            }
            else if (GetRange(3, nHD))  // 100GP Avg; 500 gp max
            {
                int nRandom = Random(13) + 1;
                switch (nRandom)
                {
                    case 1: sGem = "crpi_s_bloodston";   break;
                    case 2: sGem = "crpi_s_jasper";      break;
                    case 3: sGem = "crpi_f_alexandri";      break;
                    case 4: sGem = "crpi_f_amber";      break;
                    case 5: sGem = "crpi_f_amethyst";      break;
                    case 6: sGem = "crpi_f_chrysober";      break;
                    case 7: sGem = "crpi_f_coral";      break;
                    case 8: sGem = "crpi_f_jade";      break;
                    case 9: sGem = "crpi_f_jet";      break;
                    case 10: sGem = "crpi_f_pearl";      break;
                    case 11: sGem = "crpi_f_tourmalin";      break;
                    case 12: sGem = "crpi_p_garnet";     break;
                    case 13: sGem = "crpi_p_topaz";      break;
                }

            }
            else if (GetRange(4, nHD))  // 100 gp - 500 gp Avg.
            {
                int nRandom = d12();
                switch (nRandom)
                {
                    case 1: sGem = "crpi_f_alexandri";      break;
                    case 2: sGem = "crpi_f_amethyst";      break;
                    case 3: sGem = "crpi_f_jade";      break;
                    case 4: sGem = "crpi_f_jet";      break;
                    case 5: sGem = "crpi_f_pearl";      break;
                    case 6: sGem = "crpi_f_tourmalin";      break;
                    case 7: sGem = "crpi_p_aquamarin";      break;
                    case 8: sGem = "crpi_p_blackperl";      break;
                    case 9: sGem = "crpi_p_garnet";      break;
                    case 10: sGem = "crpi_p_peridot";      break;
                    case 11: sGem = "crpi_p_spinel";      break;
                    case 12: sGem = "crpi_p_topaz";      break;
                }
            }
            else if (GetRange(5, nHD))  // 500 gp avg; 1000 gp max
            {
                int nRandom = d8();
                switch (nRandom)
                {
                    case 1: sGem = "crpi_p_aquamarin";      break;
                    case 2: sGem = "crpi_p_blackperl";      break;
                    case 3: sGem = "crpi_p_garnet";      break;
                    case 4: sGem = "crpi_p_peridot";      break;
                    case 5: sGem = "crpi_p_spinel";      break;
                    case 6: sGem = "crpi_p_topaz";      break;
                    case 7: sGem = "crpi_g_opal";      break;
                    case 8: sGem = "crpi_g_sapphire";      break;
                }
            }
            else if (GetRange(6, nHD))// * Anything higher than level 15 1000 gp avg; any
            {
                int nRandom = d10();
                switch (nRandom)
                {
                    case 1: sGem = "crpi_g_blackopal";      break;
                    case 2: sGem = "crpi_g_blacksaph";      break;
                    case 3: sGem = "crpi_g_emerald";      break;
                    case 4: sGem = "crpi_g_fireopal";      break;
                    case 5: sGem = "crpi_g_diamond";      break;
                    case 6: sGem = "crpi_g_ruby";      break;
                    case 7: sGem = "crpi_g_sapphire";      break;
                    case 8: sGem = "crpi_g_starruby";      break;
                    case 9: sGem = "crpi_g_starsapph";      break;
                    case 10: sGem = "crpi_g_diamond";      break;
                }
            }
    ...
    void CreateJewel(object oTarget, object oAdventurer, int nTreasureType, int nModifier = 0)
    {
      ...
      //dbSpeak("Create Jewel");
       //CRP CODE ADDITION
        if(CRP_USE_BREAKABLE_ITEMS && CRP_CONTAINER_HAS_BEEN_DESTROYED){
            object oTemp = CreateItemOnObject(sJewel, oSwap);
            if(crp_GetIsItemDestroyed(oTemp, CRP_ALT_DAMAGE))
                dbCreateItemOnObject(CRP_DESTROYED_ITEM, oTarget);
            else
                dbCreateItemOnObject(sJewel, oTarget);
    ...
    void CreateDivineScroll(object oTarget, object oAdventurer, int nModifier=0)
    {
        ...
        //CRP CODE ADDITION
        if(CRP_USE_BREAKABLE_ITEMS && CRP_CONTAINER_HAS_BEEN_DESTROYED){
            object oTemp = CreateItemOnObject(sScroll, oSwap);
            if(crp_GetIsItemDestroyed(oTemp, CRP_ALT_DAMAGE))
                dbCreateItemOnObject(CRP_DESTROYED_ITEM, oTarget);
            else
                dbCreateItemOnObject(sScroll, oTarget);
    ...
    void CreateAmmo(object oTarget, object oAdventurer, int nModifier=0)
    {
        ...
        //CRP CODE ADDITION
        if(CRP_USE_BREAKABLE_ITEMS && CRP_CONTAINER_HAS_BEEN_DESTROYED){
            object oTemp = CreateItemOnObject(sAmmo, oSwap);
            if(crp_GetIsItemDestroyed(oTemp, CRP_ALT_DAMAGE))
                dbCreateItemOnObject(CRP_DESTROYED_ITEM, oTarget);
            else
                dbCreateItemOnObject(sAmmo, oTarget);
    ...
    void CreateTrapKit(object oTarget, object oAdventurer, int nModifier = 0)
    {
      ...
      string sKit = "";
        ...
        //CRP CODE ADDITION
        if(CRP_USE_BREAKABLE_ITEMS && CRP_CONTAINER_HAS_BEEN_DESTROYED){
            object oTemp = CreateItemOnObject(sKit, oSwap);
            if(crp_GetIsItemDestroyed(oTemp, CRP_ALT_DAMAGE))
                dbCreateItemOnObject(CRP_DESTROYED_ITEM, oTarget);
            else
                dbCreateItemOnObject(sKit, oTarget);
    ...
    void CreatePotion(object oTarget, object oAdventurer, int nModifier = 0)
    {
        ...
        //CRP CODE ADDITION
        if(CRP_USE_BREAKABLE_ITEMS && CRP_CONTAINER_HAS_BEEN_DESTROYED){
            object oTemp = CreateItemOnObject(sPotion, oSwap);
            if(crp_GetIsItemDestroyed(oTemp, CRP_ALT_DAMAGE))
                dbCreateItemOnObject(CRP_DESTROYED_ITEM, oTarget);
            else
                dbCreateItemOnObject(sPotion, oTarget);
    ...
    //:://////////////////////////////////////////////
        void CreateGenericMiscItem(object oTarget, object oAdventurer, int nModifier=0)
        {
            ...
            else if (GetRange(6, nHD))   // * 8000 - 25000
            {
             ...
             //CRP CODE ADDITION
             if(CRP_USE_BREAKABLE_ITEMS && CRP_CONTAINER_HAS_BEEN_DESTROYED){
               object oTemp = CreateItemOnObject(sItem, oSwap);
               if(crp_GetIsItemDestroyed(oTemp, CRP_ALT_DAMAGE))
                 dbCreateItemOnObject(CRP_DESTROYED_ITEM, oTarget);
               else
                 dbCreateItemOnObject(sItem, oTarget);
...
void GenerateNPCTreasure(int nTreasureValue=1, object oTreasureGetter=OBJECT_SELF, object oKiller=OBJECT_SELF)
{
    ...
    if (nTreasureValue == 1)
    {
        ...
        {
            if(Random(100) + 1 <= 90)
                GenerateTreasure(1, oTreasureGetter, oKiller);
        }
        else
        {   //ORIGINAL BIOWARE CODE
            if (Random(100)+1 >= 75)
