...
void main()
{
    ...
    if(GetGold(oPC) >=150)
    {
        ...
        SetLocalInt(oItem,"hrs_08",TRUE);
        SetLocalInt(oPC,"hrs_08",TRUE);

    if(GetIsObjectValid(GetItemPossessedBy(oPC,"te_lightbard")) == TRUE)
    {
        oCreature = HorseCreateHorse("hrs_08",GetLocation(oPC),oPC,"",512);
    }
    else if(GetIsObjectValid(GetItemPossessedBy(oPC,"te_medbard")) == TRUE)
    {
        oCreature = HorseCreateHorse("hrs_08",GetLocation(oPC),oPC,"",513);
    }
    else if(GetIsObjectValid(GetItemPossessedBy(oPC,"te_heabard")) == TRUE)
    {
        oCreature = HorseCreateHorse("hrs_08",GetLocation(oPC),oPC,"",514);
    }
    else
    {
        oCreature = HorseCreateHorse("hrs_08",GetLocation(oPC),oPC,"",510);
    }
