...
// meaglyn
void PrintWWinfo(object oNPC)
{
   object oPC = GetFirstPC();

   int nwp = GetLocalInt(oNPC, "AWW_WP_NUM");
   int nwn = GetLocalInt(oNPC, "AWW_WN_NUM");
   object cur = GetLocalObject(oNPC, "AWW_CURWP");

   SendMessageToPC(oPC, GetTag(oNPC) + " AWW_WP_NUM=" + IntToString(nwp)
           + " AWW_WN_NUM=" + IntToString(nwn) + " WP_CUR=" + GetTag(cur)
           + " Idx = " + IntToString(aww_GetIdxFromWP(cur)));
}
...
void main()
{
    ...
    else if( GetWalkCondition(NW_WALK_FLAG_CONSTANT) )
    {
        ...
        // meaglyn
        //SendMessageToPC(GetFirstPC(), "HB calls aww_WalkWayPoints for " + GetTag(OBJECT_SELF));
        //PrintWWinfo(OBJECT_SELF);
        aww_WalkWayPoints();
