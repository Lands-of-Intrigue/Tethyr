    ...
    switch (nEvent)
        {
            case X2_ITEM_EVENT_EQUIP:
                ...
                nAssLevel = GetLevelByClass(CLASS_TYPE_WIZARD,oPC);


                if ((nPalLevel >= 5)&&GetHasFeat(1306,oPC) == TRUE)
                {
                    SendMessageToPC(oPC, "You feel the power of your deity surge through these gloves as you put them on.");
                    // Your code goes here
                    // If the spell is cast again, any previous enhancement boni are kept

                    if(nPalLevel + nAssLevel >=5)
                    {
                    ...
                    {
                        IPSafeAddItemProperty(oItem, ItemPropertyEnhancementBonus(1), 0.0, X2_IP_ADDPROP_POLICY_KEEP_EXISTING,TRUE);
                        IPSafeAddItemProperty(oItem, ItemPropertyDamageBonus(IP_CONST_DAMAGETYPE_MAGICAL,IP_CONST_DAMAGEBONUS_1), 0.0, X2_IP_ADDPROP_POLICY_KEEP_EXISTING,TRUE);
                    }
                }
                break;
