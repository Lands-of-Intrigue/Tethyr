...
string sTreasureValueVar = "X0_TREASURE_VALUE"; // henesua - added value
...
void CTG_InitContainer(object oBaseCont)
{
    ...
    if(nItems>999)
        WriteTimestampedLogEntry("WARNING: MAGUS LOOT SYSTEM: x0_i0_treasure CTG_initContainer("+GetTag(oBaseCont)+"). Container has more than 999 items. Items 1000-"+IntToString(nItems)+" will not be spawned.");

    int i;
    object oTmp = OBJECT_INVALID;
    string sID;  // henesua - mod var names
    oTmp = GetFirstItemInInventory(oBaseCont);
    for (i=1; i <= nItems && GetIsObjectValid(oTmp); i++)
    {
        // Store the item and its base type as local vars on the container object itself.
        sID       = IntToString(i); // henesua - bug fix of var names
        SetLocalInt(oBaseCont, sBaseTypeVarname+sID, GetBaseItemType(oTmp)); // base item type
        SetLocalObject(oBaseCont, sTreasureItemVarname+sID, oTmp);           // object ref of item

        // henesua added
        SetLocalInt(oBaseCont, sTreasureValueVar+sID, GetGoldPieceValue(oTmp)); // value of item
                                        ...
                                        int nBaseType3=BASE_ITEM_INVALID)
{
    ...
    if (nTreasureType == TREASURE_TYPE_UNIQUE) {
        ...
        int nRandom = Random(nItemsInBaseCont)+1;
...
//            CreateItemOnObject(GetResRef(oItem), oCont);
    ...
    while (nItemsCreated < nItems) {
        ...
        {
            nRandom = Random(nItemsInBaseCont)+1;
