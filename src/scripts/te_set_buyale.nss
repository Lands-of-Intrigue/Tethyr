...
void main()
{
    ...
    if( (GetLocalInt(oPC,TRADE_CRATESHADEALE) >= 1 ) || ( GetItemPossessedBy(oPC,TRADE_CRATESHADEALE) != OBJECT_INVALID) )
    {
        object oItem = GetItemPossessedBy(oPC,TRADE_CRATESHADEALE);
        DestroyObject(oItem);
        nGold = FloatToInt(VALUE_CRATESHADEALE*fMerchant);
