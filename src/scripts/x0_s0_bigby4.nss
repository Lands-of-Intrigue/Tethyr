...
void RunHandImpact(object oTarget, object oCaster)
{
    ...
    int nCasterModifiers = GetCasterAbilityModifier(oCaster) + TE_GetCasterLevel(OBJECT_SELF,GetLastSpellCastClass());
    ...
    */
    ...
    if(!GetIsReactionTypeFriendly(oTarget))
    {
        ...
        if(nResult  == 0)
        {
            ...
            //----------------------------------------------------------
            SetLocalInt(oTarget,"XP2_L_SPELL_SAVE_DC_" + IntToString (nSpellID), TE_GetSpellSaveDC(OBJECT_SELF,GetSpellId(),GetLastSpellCastClass()));
