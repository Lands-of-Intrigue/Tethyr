...
void AddChillingEffectToWeapon(object oTarget, float fDuration)
{
...
void main()
{
        ...
        if(nItem == TRUE)
        {
            if(GetIsObjectValid(oMyWeapon))
            {
                ...
                if (nDuration > 0)
                {
                    ...
                    SetLocalInt(oMyWeapon,"X2_SPELL_CLEVEL_CHILLING_WEAPON",nCasterLvl);
                    ApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, GetItemPossessor(oMyWeapon));
                    ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eDur, GetItemPossessor(oMyWeapon), RoundsToSeconds(nDuration));
                    itemproperty iIp = ItemPropertyDamageBonus(IP_CONST_DAMAGETYPE_MAGICAL, IP_CONST_DAMAGEBONUS_1d6);
    ...
    {
        DoGrenade(d8(1),d4(1)+1, VFX_IMP_MAGBLUE, VFX_FNF_MYSTICAL_EXPLOSION, DAMAGE_TYPE_SONIC, RADIUS_SIZE_LARGE, OBJECT_TYPE_CREATURE | OBJECT_TYPE_DOOR | OBJECT_TYPE_PLACEABLE);
    }
}
