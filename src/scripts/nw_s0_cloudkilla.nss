...
void main()
{
    ...
    if(spellsIsTarget(oTarget,SPELL_TARGET_STANDARDHOSTILE , GetAreaOfEffectCreator()) )
    {
        ...
        {
            //Make SR Check
            if(!MyResistSpell(GetAreaOfEffectCreator(), oTarget, fDelay)|| GetTag(oTarget) == "te_gol_clay")
            {
                ...
                {
                    //Make a save or die
                    if(!MySavingThrow(SAVING_THROW_FORT, oTarget, TE_GetSpellSaveDC(OBJECT_SELF,GetSpellId(),GetLastSpellCastClass()), SAVING_THROW_TYPE_DEATH, OBJECT_SELF, fDelay))
                    {
                        DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_INSTANT, eDeath, oTarget));
                        DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, oTarget));
                    }
                    else
                    {
                        ...
                        if(!MySavingThrow(SAVING_THROW_FORT, oTarget, TE_GetSpellSaveDC(OBJECT_SELF,GetSpellId(),GetLastSpellCastClass()), SAVING_THROW_TYPE_NONE, OBJECT_SELF, fDelay))
                        {
