...
// End of Spell Cast Hook
    ...
    effect eVis = EffectVisualEffect(1740);
        ...
        if(!GetIsReactionTypeFriendly(oTarget))
            {
                ...
                eRay = EffectBeam(VFX_BEAM_HOLY, OBJECT_SELF, BODY_NODE_CHEST);
                if (!MyResistSpell(OBJECT_SELF, oTarget))
                {
                    //Make a saving throw check
                    if(/*Will Save*/ MySavingThrow(SAVING_THROW_WILL, oTarget, TE_GetSpellSaveDC(OBJECT_SELF,GetSpellId(),GetLastSpellCastClass()), SAVING_THROW_TYPE_DIVINE))
                    {
                        nDamage = nDamage/2;
                    }
    ...
    {
        eRay = EffectBeam(VFX_BEAM_HOLY, OBJECT_SELF, BODY_NODE_CHEST,TRUE);
        ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eRay, oTarget, 1.7);
    }
}
