                ...
                if (spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, OBJECT_SELF))
                {
                   ...
                   if (!MyResistSpell(OBJECT_SELF, oTarget) || nEssence == 1059)
                   {
                        ...
                        else if(nEssence == 1059)//Vitriolic
                        {
                            ...
                            ApplyEffectToObject(DURATION_TYPE_TEMPORARY,eBeamS,oTarget,1.0);
                        }
                   ...
                   ApplyEffectToObject(DURATION_TYPE_TEMPORARY,eBeamS,oTarget,1.0);
                   oNextTarget = oTarget;
                    if(nEssence == 1055){eBeamS = EffectBeam(VFX_BEAM_EVIL, oNextTarget, BODY_NODE_CHEST);}
                    else if(nEssence == 1056){eBeamS = EffectBeam(VFX_BEAM_MIND, oNextTarget, BODY_NODE_CHEST);}
                    else if(nEssence == 1057){eBeamS = EffectBeam(VFX_BEAM_FIRE, oNextTarget, BODY_NODE_CHEST);}
                    else if(nEssence == 1058){eBeamS = EffectBeam(VFX_BEAM_COLD, oNextTarget, BODY_NODE_CHEST);}
                    else if(nEssence == 1059){eBeamS = EffectBeam(VFX_BEAM_DISINTEGRATE, oNextTarget, BODY_NODE_CHEST);}
                    else if(nEssence == 1060){eBeamS = EffectBeam(VFX_BEAM_MIND, oNextTarget, BODY_NODE_CHEST);}
                    else if(nEssence == 1061){eBeamS = EffectBeam(VFX_BEAM_BLACK, oNextTarget, BODY_NODE_CHEST);}
                    else if(nEssence == 1062){eBeamS = EffectBeam(VFX_BEAM_CHAIN, oNextTarget, BODY_NODE_CHEST);}
                    else                     {eBeamS = EffectBeam(VFX_BEAM_ODD, oNextTarget, BODY_NODE_CHEST);}
                }
