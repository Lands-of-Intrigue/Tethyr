...
// End of Spell Cast Hook
    ...
    int nHD = d4(TE_GetCasterLevel(OBJECT_SELF,GetLastSpellCastClass())); //Roll to see how many HD worth of creature will be killed
    int nMetaMagic = GetMetaMagicFeat();
    int nCurrentHD;
    int bAlreadyAffected;
    int nMax = 9;// maximun hd creature affected, set this to 9 so that a lower HD creature is chosen automatically
    ...
    {
        nHD = 4 * TE_GetCasterLevel(OBJECT_SELF,GetLastSpellCastClass());//Damage is at max
    }
    ...
    while ((nHD > 0) && (bContinueLoop))
    {
        ...
        if(bContinueLoop == TRUE)
        {
            ...
            if(!MyResistSpell(OBJECT_SELF, oLowest, fDelay))
            {
                ...
                if(!MySavingThrow(SAVING_THROW_FORT, oLowest, TE_GetSpellSaveDC(OBJECT_SELF,GetSpellId(),GetLastSpellCastClass()), SAVING_THROW_TYPE_DEATH, OBJECT_SELF, fDelay))
                {
