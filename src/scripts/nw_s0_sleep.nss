    ...
    while ((nHD > 0) && (bContinueLoop))
    {
        ...
        while (GetIsObjectValid(oTarget))
        {
            ...
            if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF)
                && GetRacialType(oTarget) != RACIAL_TYPE_CONSTRUCT && GetIsUndead(oTarget) != TRUE)
            {
        ...
        if(oLowest != OBJECT_INVALID)
        {
            ...
            {
                //Make Fort save
                if(!MySavingThrow(SAVING_THROW_WILL, oLowest, TE_GetSpellSaveDC(OBJECT_SELF,GetSpellId(),GetLastSpellCastClass()), SAVING_THROW_TYPE_MIND_SPELLS))
                {
