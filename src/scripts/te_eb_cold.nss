...
void main()
{
    ...
    effect eVis = EffectVisualEffect(VFX_IMP_FROST_S);                        //Thing that appears on target when hit.
    effect xEff = EffectMovementSpeedDecrease(50);                            //Secondary effect for flavor.
    effect eBeamF = EffectBeam(VFX_BEAM_COLD , oPC, BODY_NODE_HAND, TRUE);
    effect eBeamS = EffectBeam(VFX_BEAM_COLD , oPC, BODY_NODE_HAND, FALSE);   //Beam Effect. Edit top beam effect as well.
    ...
    else
    {
        ...
        IPSafeAddItemProperty(oMyWeapon,ItemPropertyDamageBonus(IP_CONST_DAMAGETYPE_COLD,IP_CONST_DAMAGEBONUS_2d6), TurnsToSeconds(CL), X2_IP_ADDPROP_POLICY_REPLACE_EXISTING,FALSE,TRUE);
