...
void main()
{
    ...
    float fBeyondSkyTemperature     = GetLocalFloat(oModule,WEATHER_BEYOND_SKY_TEMPERATURE);
    float fLocalGroundTemperature   = GetLocalFloat(oModule,WEATHER_LOCAL_GROUND_TEMPERATURE);
    float fBeyondGroundTemperature   = GetLocalFloat(oModule,WEATHER_BEYOND_GROUND_TEMPERATURE);

    SetLocalFloat(oModule, WEATHER_LOCAL_SKY_TEMPERATURE,  fLocalSkyTemperature     -5.0f);
    SetLocalFloat(oModule, WEATHER_BEYOND_SKY_TEMPERATURE, fBeyondSkyTemperature    -5.0f);
    SetLocalFloat(oModule, WEATHER_LOCAL_GROUND_TEMPERATURE, fLocalGroundTemperature  -5.0f);
    SetLocalFloat(oModule, WEATHER_BEYOND_GROUND_TEMPERATURE, fBeyondGroundTemperature -5.0f);
}
