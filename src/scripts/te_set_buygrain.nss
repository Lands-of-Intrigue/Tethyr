...
void main()
{
    ...
    if( (GetLocalInt(oPC,TRADE_GRAIN) >= 1 ) || ( GetItemPossessedBy(oPC,TRADE_GRAIN) != OBJECT_INVALID) )
    {
        object oItem = GetItemPossessedBy(oPC,TRADE_GRAIN); DestroyObject(oItem);
        nGold = FloatToInt(VALUE_GRAIN*fMerchant);
