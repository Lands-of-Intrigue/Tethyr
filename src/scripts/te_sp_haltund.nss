...
// End of Spell Cast Hook
    ...
    effect eParal = EffectMovementSpeedDecrease(100);
    ...
    oTarget = GetFirstObjectInShape(SHAPE_SPHERE, 11.0, GetLocation(oTarget), TRUE, OBJECT_TYPE_CREATURE);
    //Cycle through the targets within the spell shape until an invalid object is captured.
    while(GetIsObjectValid(oTarget) && nCounter < 3)
    {
        nCounter = nCounter + 1;
        if(!GetIsReactionTypeFriendly(oTarget))
        {
            ...
            //Make sure the target is a undead
            if (GetIsUndead(oTarget))
            {
                ...
                {
                    if(
                        (!GetIsPC(oTarget) && GetLocalInt(OBJECT_SELF,"Intelligence") > 3) ||
                        (GetIsPC(oTarget))
                    )
                    {
                        //Make Will save
                        if (!/*Will Save*/ MySavingThrow(SAVING_THROW_WILL, oTarget, TE_GetSpellSaveDC(OBJECT_SELF,GetSpellId(),GetLastSpellCastClass())))
                        {
