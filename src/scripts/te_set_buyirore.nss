...
void main()
{
    ...
    if( (GetLocalInt(oPC,TRADE_IRONORE) >= 10 ) || ( GetItemPossessedBy(oPC,TRADE_IRONORE) != OBJECT_INVALID) )
    {
        object oItem = GetFirstItemInInventory(oPC);
        int nCount = 0;
        while (oItem != OBJECT_INVALID)
        {
            if((GetTag(oItem) == TRADE_IRONORE) && nCount < 10)
            {
                DestroyObject(oItem);
                nCount += 1;
            }
        ...
        nGold = FloatToInt(VALUE_IRONORE*fMerchant);
