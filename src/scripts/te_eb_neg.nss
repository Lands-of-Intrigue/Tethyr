...
void main()
{
    ...
    effect eVis = EffectVisualEffect(VFX_IMP_NEGATIVE_ENERGY);                //Thing that appears on target when hit.
    effect xEff = EffectAbilityDecrease(ABILITY_CONSTITUTION, 1);             //Secondary effect for flavor.
    effect eBeamF = EffectBeam(VFX_BEAM_EVIL , oPC, BODY_NODE_HAND, TRUE);
    effect eBeamS = EffectBeam(VFX_BEAM_EVIL , oPC, BODY_NODE_HAND, FALSE);   //Beam Effect. Edit top beam effect as well.
        ...
        if (nTouch >= 1)  //If successful...
        {
            ...
            if (SS = 1)
            {
                ...
                else
                {
                    ...
                    DelayCommand(0.2, ApplyEffectToObject(DURATION_TYPE_INSTANT, eDam, oTarget));
                    DelayCommand(0.5, ApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, oTarget));
                    DelayCommand(0.5, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectAbilityIncrease(ABILITY_CONSTITUTION, 1), oTarget, xDur));
                }
            }
    ...
    else
    {
        ...
        IPSafeAddItemProperty(oMyWeapon,ItemPropertyVisualEffect(ITEM_VISUAL_EVIL), TurnsToSeconds(CL), X2_IP_ADDPROP_POLICY_REPLACE_EXISTING,FALSE,TRUE);
    }
}
