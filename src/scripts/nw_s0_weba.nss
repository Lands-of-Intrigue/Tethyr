...
//Bug fix function
void FixDex(object oTarget)
{
if(GetAbilityScore(oTarget, ABILITY_DEXTERITY) == 3)
    {
    ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectAbilityIncrease(ABILITY_DEXTERITY, 1), oTarget, 3.0f);
    }
}
    ...
    if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, GetAreaOfEffectCreator()))
    {
         if(!GetHasFeat(FEAT_WOODLAND_STRIDE, oTarget) &&(GetCreatureFlag(oTarget, CREATURE_VAR_IS_INCORPOREAL) != TRUE) )
        {
            ...
            if(!MyResistSpell(GetAreaOfEffectCreator(), oTarget))
            {
                ...
                if(!MySavingThrow(SAVING_THROW_REFLEX, oTarget, TE_GetSpellSaveDC(OBJECT_SELF,GetSpellId(),GetLastSpellCastClass())))
                {
                    //Entangle effect and Web VFX impact
                    ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eLink, oTarget, RoundsToSeconds(1));
                    //Bug fix
                    DelayCommand(RoundsToSeconds(1)+0.1,FixDex(oTarget));
                }
