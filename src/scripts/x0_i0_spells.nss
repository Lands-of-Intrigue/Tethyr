   ...
   // SpawnScriptDebugger();
    ...
    if (spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, GetAreaOfEffectCreator()))
    {
        ...
        if(!MyResistSpell(GetAreaOfEffectCreator(), oTarget))
        {
            ...
            if(!MySavingThrow(SAVING_THROW_FORT, oTarget, TE_GetSpellSaveDC(GetAreaOfEffectCreator(),GetSpellId(),GetLastSpellCastClass()), SAVING_THROW_ALL, GetAreaOfEffectCreator()))
            {
    ...
    if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, GetAreaOfEffectCreator()))
    {
        ...
        if(!MyResistSpell(GetAreaOfEffectCreator(), oTarget, fDelay))
        {
           ...
           if (GetHasSpellEffect(SPELL_SPIKE_GROWTH, oTarget) == FALSE)
           {
                ...
                if(!MySavingThrow(SAVING_THROW_REFLEX, oTarget, TE_GetSpellSaveDC(OBJECT_SELF,GetSpellId(),GetLastSpellCastClass()), SAVING_THROW_ALL, GetAreaOfEffectCreator(), fDelay))
                {
    ...
    if (nTouch >0 )
    {
        if(!GetIsReactionTypeFriendly(oTarget))
        {
            ...
            if (!MyResistSpell(OBJECT_SELF, oTarget))
            {
                ...
                if(MySavingThrow(SAVING_THROW_WILL, oTarget, TE_GetSpellSaveDC(OBJECT_SELF,GetSpellId(),GetLastSpellCastClass()), SAVING_THROW_ALL,OBJECT_SELF))
                {
                ...
                if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay))
                {
                    for (i=1; i <= nExtraMissiles + nRemainder; i++)
                    {
                        ...
                        if(nReflexSave)
                        {
                            ...
                            {
                                nDam = GetReflexAdjustedDamage(nDam, oTarget, TE_GetSpellSaveDC(OBJECT_SELF,GetSpellId(),GetLastSpellCastClass()), SAVING_THROW_TYPE_ELECTRICITY);
                            }
                            else if ( nDAMAGETYPE == DAMAGE_TYPE_FIRE )
                            {
                                nDam = GetReflexAdjustedDamage(nDam, oTarget, TE_GetSpellSaveDC(OBJECT_SELF,GetSpellId(),GetLastSpellCastClass()), SAVING_THROW_TYPE_FIRE);
                            }
                        }
    ...
    while(GetIsObjectValid(oTarget))
    {
        if (spellsIsTarget(oTarget, nTargetType, oCaster) == TRUE)
        {
          ...
          if(!nResistSpellSuccess)
          {
                ...
                {
                  nSavingThrowSuccess = MySavingThrow(nSavingThrowType, oTarget, TE_GetSpellSaveDC(OBJECT_SELF,GetSpellId(),GetLastSpellCastClass()), nSavingThrowSubType);
                }
...
void spellsStinkingCloud(object oTarget = OBJECT_INVALID)
{
    ...
    if (GetIsObjectValid(oTarget) == TRUE)
    {
        if(spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE,  GetAreaOfEffectCreator()))
        {
            ...
            SignalEvent(oTarget, EventSpellCastAt(OBJECT_SELF, GetSpellId()));
            //Make a Fort Save
            if(!MySavingThrow(SAVING_THROW_FORT, oTarget, TE_GetSpellSaveDC(OBJECT_SELF,GetSpellId(),GetLastSpellCastClass()), SAVING_THROW_TYPE_POISON))
            {
