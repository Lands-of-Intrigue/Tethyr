...
void main()
{
    ...
    if( (GetLocalInt(oPC,TRADE_FLOUR) >= 1 ) || ( GetItemPossessedBy(oPC,TRADE_FLOUR) != OBJECT_INVALID) )
    {
        object oItem = GetItemPossessedBy(oPC,TRADE_FLOUR); DestroyObject(oItem);
        nGold = FloatToInt(VALUE_FLOUR*fMerchant);
