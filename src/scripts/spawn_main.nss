...
// The Spawn Function - [File: spawn_main]
void Spawn();
// This Function Processes a Spawn - [File: spawn_main]
void ProcessSpawn(object oSpawn, int nProcessFrequency, int nPCCount, int nTimeNow, int nWaypointCount);
// This Function Performs the Spawn - [File: spawn_main]
void DoSpawn(object oSpawn, int nTimeNow);
// This Function Spawns a Camp - [File: spawn_main]
object CampSpawn(object oSpawn, string sCamp, location lCamp);
// This Function Spawns the Camp Members - [File: spawn_main]
...
void Spawn()
{
    ...
    // Check Area State
    /* // handled by area with local integer "AREA_NESS_DISABLE"
    ...
    */
    ...
    for (nNth = 1; nNth <= nSpawns; nNth++)
    {
        ...
        if (nPCCount > 0)
        {
            if(     GetLocalInt(oSpawn, "SpawnNumSavedStates") > 0
                ||  GetLocalInt(oSpawn, "SpawnNumSavedCampStates") > 0
              )
            {
        ...
        else if (nCurrentProcessTick == 0)
        {
            ...
            nProcessSpawn = TRUE;
            SetLocalInt(oSpawn, "CurrentProcessTick", 2-GetLocalInt(oSpawn, "f_ProcessOffset"));
            //debug("Tick 1");
            //debug("+");
        }
        else
        ...
        {
            DelayCommand(0.0, ProcessSpawn(oSpawn, nProcessFrequency, nPCCount, nTimeNow, nNth));
        }
...
void ProcessSpawn(object oSpawn, int nProcessFrequency, int nPCCount, int nTimeNow, int nWaypoint)
{
    ...
    int bAppear         = GetLocalInt(oSpawn, "f_Appear"); // Magus - use appear/disappear animation
    ...
    for (nChildSlot = 1; nChildSlot <= nSpawnNumber; nChildSlot++)
    {
        ...
        {
            SetLocalObject(oCreature, "ParentSpawn", oSpawn);
        ...
        if (nSpawnChild == TRUE)
        {
            ...
            if(sNPCSchedule!="")
            {
                string character_id = GetLocalString(oSpawn,"creature_id");
                if(character_id=="")
                {
                    character_id    = GetPCID(oCreature);

                    SetLocalString(oSpawn,"creature_id", character_id);
                }

                ExecuteScript(sNPCSchedule,oSpawn);

                if(GetCreatureLocationState(character_id)!=GetLocalInt(oSpawn,"NPC_LOCATION_ID"))
                {
                    nDespawning = TRUE;
                    nSpawnDespawn = TRUE;
                }
            ...
            // Random Walk
            if (nRandomWalk && !nDespawning && !nSpawnDespawn)
            {
                if(     nAction != ACTION_WAIT
                  ...
                  )
                {
                    if (    nAction==ACTION_RANDOMWALK
                            && fWanderRange > 0.0
                            && GetDistanceBetweenLocations(lHome, GetLocation(oCreature)) > fWanderRange
                        )
                    {
            ...
            // Patrol
            if(nPatrolRoute>-1 && !nDespawning && !nSpawnDespawn)
            {
                if (!GetIsInCombat(oCreature) && !IsInConversation(oCreature))
                {
                    nPatrolScriptRunning = GetLocalInt(oCreature, "PatrolScriptRunning");
                    if(GetLocalInt(OBJECT_SELF, "DELAY_PATROL_END")>GetCurrentRealSeconds())
                    {
                        // we are delaying - its ok
                    }
                    else if(     nAction==ACTION_INVALID
                            &&  !nPatrolScriptRunning
                      )
                    {
                        if(bCheckForStuckPatrols)
            ...
            // ReturnHome
            if (nReturnHome && !nDespawning && !nSpawnDespawn)
            {
                if (GetDistanceBetweenLocations(lHome, GetLocation(oCreature)) > fReturnHomeRange)
                {
                    if(     ( nAction==ACTION_INVALID || nAction==ACTION_RANDOMWALK )
                        &&  !GetIsInCombat(oCreature)
                        &&  !IsInConversation(oCreature)
                      )
                    {
            ...
            if (nSpawnSit && !nDespawning)
            {
                if (GetCurrentAction(oCreature) != ACTION_SIT)
                {
                    ...
                    {
                        if(nSpawnSitGround)
                            AssignCommand(oCreature, ActionPlayAnimation(ANIMATION_LOOPING_SIT_CROSS, 1.0, 10000.0) );
                        else
        ...
        // MAGUS -- explicit Despawn of creatures and camps when NESS turned off
        if(     GetLocalInt(GetArea(oSpawn), "AREA_NESS_DISABLE")
            &&  !nPlaceable && !nSpawnItem && !nSpawnMerchant
          )
            nSpawnDespawn == TRUE;
                                     ...
                                     ! GetIsDM(GetMaster(oCreature)))
        {
                ...
                || nSpawnMerchant == TRUE)
            {
                ...
                if (! (nSaveState && nSpawnCamp))
                {
                    ...
                    // HENESUA - creature that show up with appear animation, despawn with disappear
                    if(bAppear)
                        ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectDisappear(), oCreature);
                    else
            ...
            else  // is not placeable, camp, item, or merchant
            {
                   ...
                   || (nPCCheck == TRUE && nPCCount == 0))
                {
                    ...
                    if (nEntranceExit > -1)
                    {
                        ...
                        AssignCommand(oCreature, SetIsDestroyable(TRUE, TRUE, FALSE));
                        SpawnCountDebug(oSpawn, "despawning " + ObjectToString(oCreature));

                        // HENESUA - creatures that show up with appear animation, despawn with disappear
                        if(bAppear)
                            AssignCommand(oCreature, ActionDoCommand( ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectDisappear(), oCreature) ));
                        else
...
void DoSpawn(object oSpawn, int nTimeNow)
{
    ...
    effect eSpawn, eArea;
    int nObjectType; // int nRadiusValid;
    ...
    int bAppear                 = GetLocalInt(oSpawn, "f_Appear"); // Magus - use appear animation
    ...
    if (sTemplate != "")
    {
        ...
        else
        {
            ...
            /*
            if(sNewTag=="")
                oSpawned = CreateObject(nObjectType, sTemplate, lSpawnLocation, bAppear);
            else
            */
                oSpawned = CreateObject(nObjectType, sTemplate, lSpawnLocation, bAppear, sNewTag);

                // check unique
                if(GetLocalInt(oSpawned,"UNIQUE"))
                {
                    SetLocalInt(oSpawn,"UNIQUE",TRUE);
