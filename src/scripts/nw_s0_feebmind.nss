...
// End of Spell Cast Hook
    ...
    int nDuration = TE_GetCasterLevel(OBJECT_SELF,GetLastSpellCastClass())/2;
    int nLoss = TE_GetCasterLevel(OBJECT_SELF,GetLastSpellCastClass())/4;
    ...
    if(!GetIsReactionTypeFriendly(oTarget))
    {
        ...
        if (!MyResistSpell(OBJECT_SELF, oTarget))
        {
            ...
            int nWillResult =  WillSave(oTarget, TE_GetSpellSaveDC(OBJECT_SELF,GetSpellId(),GetLastSpellCastClass()), SAVING_THROW_TYPE_MIND_SPELLS);
            if (nWillResult == 0)
            {
