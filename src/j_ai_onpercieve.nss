...
void main()
{
      ...
      !GetIgnore(oPerceived))
    {
        if(GetIsPC(oPerceived) == TRUE &&
            GetDistanceBetween(oPerceived, OBJECT_SELF)<5.0&&
            (GetTag(oPerceived) == "te_sylvan_002"||
             GetTag(oPerceived) == "te_sylvan_005"||
             GetTag(oPerceived) == "te_sylvan_006"||
             GetTag(oPerceived) == "te_sylvan_010"||
             GetTag(oPerceived) == "te_sylvan_027"||
             GetTag(oPerceived) == "te_sylvan_029"||
             GetTag(oPerceived) == "te_sylvan_034"||
             GetTag(oPerceived) == "te_sylvan_033"||
             GetTag(oPerceived) == "te_sylvan_060"))
        {
            ChangeFaction(OBJECT_SELF,GetObjectByTag("I_HostAnimal"));
        }
