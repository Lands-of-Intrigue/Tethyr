...
//CUSTOMIZED AI SETTINGS
    if(GetLocalInt(OBJECT_SELF,"DeathEffect") != 0)  {SetAIConstant(AI_DEATH_VISUAL_EFFECT,GetLocalInt(OBJECT_SELF,"DeathEffect"));}
    if(GetLocalInt(OBJECT_SELF,"Lycan") != 0)        {SetAIConstant(AI_POLYMORPH_INTO, GetLocalInt(OBJECT_SELF,"Lycan"));}
    if(GetLocalInt(OBJECT_SELF,"NonHostile") != 0)   {SetSpawnInCondition(AI_FLAG_OTHER_ONLY_ATTACK_IF_ATTACKED, AI_OTHER_MASTER);}
    if(GetLocalInt(OBJECT_SELF,"FavoredRace") != 0)  {SetAIConstant(AI_FAVOURED_ENEMY_RACE, GetLocalInt(OBJECT_SELF,"FavoredRace"));}
    if(GetLocalInt(OBJECT_SELF,"FavoredClass") != 0) {SetAIConstant(AI_FAVOURED_ENEMY_CLASS, GetLocalInt(OBJECT_SELF,"FavoredClass"));}
    if(GetLocalInt(OBJECT_SELF,"NeverBreak") != 0)   {SetSpawnInCondition(AI_FLAG_FLEEING_FEARLESS, AI_TARGETING_FLEE_MASTER);}

    if(GetLocalInt(OBJECT_SELF,"Intelligence") != 0)
    {SetAIInteger(AI_INTELLIGENCE,GetLocalInt(OBJECT_SELF,"Intelligence"));}

    if(GetLocalInt(OBJECT_SELF,"Morale") != 0)
    {SetAIInteger(AI_MORALE, GetLocalInt(OBJECT_SELF,"Morale"));}

    SetAIInteger(AI_MAX_TURNS_TO_ATTACK_ONE_TARGET, 12);
    SetSpawnInCondition(AI_FLAG_OTHER_DONT_RESPOND_TO_EMOTES, AI_OTHER_MASTER);
    SetSpawnInCondition(AI_FLAG_OTHER_REST_AFTER_COMBAT, AI_OTHER_MASTER);
    SetSpawnInCondition(AI_FLAG_COMBAT_NO_GO_FOR_THE_KILL, AI_COMBAT_MASTER);

    if(GetLocalString(OBJECT_SELF,"Archetype") == "BossMelee")
    {
        SetAIInteger(AI_MELEE_LAST_TO_NEW_TARGET_CHANCE, 20);
        AI_SetAITargetingValues(TARGETING_HP_MAXIMUM, TARGET_LOWER, 1, 3);
        SetSpawnInCondition(AI_FLAG_FLEEING_FEARLESS, AI_TARGETING_FLEE_MASTER);
        ...
        SetSpawnInCondition(AI_FLAG_OTHER_COMBAT_GROUP_LEADER, AI_OTHER_COMBAT_MASTER);
        SetSpawnInCondition(AI_FLAG_OTHER_COMBAT_BOSS_MONSTER_SHOUT, AI_OTHER_COMBAT_MASTER);
        SetAIInteger(AI_BOSS_MONSTER_SHOUT_RANGE, 120);
        SetSpawnInCondition(AI_FLAG_OTHER_LAG_NO_SPELLS, AI_OTHER_MASTER);
        SetAIConstant(LAG_AI_LEVEL_NO_PC_OR_ENEMY_50M, AI_LEVEL_VERY_LOW);
    }
    else if(GetLocalString(OBJECT_SELF,"Archetype") == "BossMage")
    {
        SetAIInteger(AI_SPELL_LAST_TO_NEW_TARGET_CHANCE, 20);
        AI_SetAITargetingValues(TARGETING_HP_CURRENT, TARGET_LOWER, 1, 3);
        SetSpawnInCondition(AI_FLAG_FLEEING_FEARLESS, AI_TARGETING_FLEE_MASTER);
    ...
    else if(GetLocalString(OBJECT_SELF,"Archetype") == "BossRange")
    {
        SetAIInteger(AI_RANGED_LAST_TO_NEW_TARGET_CHANCE, 20);
        AI_SetAITargetingValues(TARGETING_HP_CURRENT, TARGET_LOWER, 1, 3);
        SetSpawnInCondition(AI_FLAG_FLEEING_FEARLESS, AI_TARGETING_FLEE_MASTER);
    ...
    else if(GetLocalString(OBJECT_SELF,"Archetype") == "SLMelee")
    {
        SetAIInteger(AI_MELEE_LAST_TO_NEW_TARGET_CHANCE, 20);
        SetSpawnInCondition(AI_FLAG_COMBAT_BETTER_AT_HAND_TO_HAND, AI_COMBAT_MASTER);
        SetSpawnInCondition(AI_FLAG_COMBAT_NO_GO_FOR_THE_KILL, AI_COMBAT_MASTER);
    ...
    else if(GetLocalString(OBJECT_SELF,"Archetype") == "SLMage")
    {
        SetAIInteger(AI_SPELL_LAST_TO_NEW_TARGET_CHANCE, 20);
        SetSpawnInCondition(AI_FLAG_COMBAT_ARCHER_ATTACKING, AI_COMBAT_MASTER);
        SetSpawnInCondition(AI_FLAG_COMBAT_ARCHER_ALWAYS_MOVE_BACK, AI_COMBAT_MASTER);
    ...
    else if(GetLocalString(OBJECT_SELF,"Archetype") == "SLRange")
    {
        SetAIInteger(AI_RANGED_LAST_TO_NEW_TARGET_CHANCE, 20);
        SetSpawnInCondition(AI_FLAG_COMBAT_ARCHER_ATTACKING, AI_COMBAT_MASTER);
        SetSpawnInCondition(AI_FLAG_COMBAT_ARCHER_ALWAYS_MOVE_BACK, AI_COMBAT_MASTER);
    ...
    else if(GetLocalString(OBJECT_SELF,"Archetype") == "Melee")
    {
        SetAIInteger(AI_MELEE_LAST_TO_NEW_TARGET_CHANCE, 20);
        SetSpawnInCondition(AI_FLAG_FLEEING_NEVER_FIGHT_IMPOSSIBLE_ODDS, AI_TARGETING_FLEE_MASTER);
        SetAIInteger(AMOUNT_OF_HD_DIFFERENCE_TO_CHECK, -10);
        SetAIInteger(HP_PERCENT_TO_CHECK_AT, 10);
        SetAIInteger(AI_DAMAGE_AT_ONCE_FOR_MORALE_PENALTY, GetMaxHitPoints()/3);
        SetAIInteger(AI_DAMAGE_AT_ONCE_PENALTY, 6);
        SetSpawnInCondition(AI_FLAG_FLEEING_FLEE_TO_NEAREST_NONE_SEEN, AI_TARGETING_FLEE_MASTER);
        SetAIInteger(AI_RANGED_WEAPON_RANGE, 5);
        SetSpawnInCondition(AI_FLAG_COMBAT_BETTER_AT_HAND_TO_HAND, AI_COMBAT_MASTER);
        SetSpawnInCondition(AI_FLAG_OTHER_LAG_NO_SPELLS, AI_OTHER_MASTER);
        SetAIConstant(LAG_AI_LEVEL_NO_PC_OR_ENEMY_50M, AI_LEVEL_VERY_LOW);
    }
    else if(GetLocalString(OBJECT_SELF,"Archetype") == "Range")
    {
        SetAIInteger(AI_RANGED_LAST_TO_NEW_TARGET_CHANCE, 20);
        SetSpawnInCondition(AI_FLAG_FLEEING_NEVER_FIGHT_IMPOSSIBLE_ODDS, AI_TARGETING_FLEE_MASTER);
        SetAIInteger(AMOUNT_OF_HD_DIFFERENCE_TO_CHECK, -10);
    ...
    else if(GetLocalString(OBJECT_SELF,"Archetype") == "Sneak")
    {
        SetAIInteger(AI_MELEE_LAST_TO_NEW_TARGET_CHANCE, 20);
        SetSpawnInCondition(AI_FLAG_FLEEING_NEVER_FIGHT_IMPOSSIBLE_ODDS, AI_TARGETING_FLEE_MASTER);
        SetAIInteger(AMOUNT_OF_HD_DIFFERENCE_TO_CHECK, -10);
        ...
        SetSpawnInCondition(AI_FLAG_COMBAT_BETTER_AT_HAND_TO_HAND, AI_COMBAT_MASTER);
        SetSpawnInCondition(AI_FLAG_OTHER_SEARCH_IF_ENEMIES_NEAR, AI_OTHER_MASTER);
        SetAIInteger(AI_SEARCH_IF_ENEMIES_NEAR_RANGE, 20);
        SetSpawnInCondition(AI_FLAG_OTHER_LAG_NO_SPELLS, AI_OTHER_MASTER);
        SetAIConstant(LAG_AI_LEVEL_NO_PC_OR_ENEMY_50M, AI_LEVEL_VERY_LOW);
    }
    else if(GetLocalString(OBJECT_SELF,"Archetype") == "Cleric")
    {
        SetAIInteger(AI_SPELL_LAST_TO_NEW_TARGET_CHANCE, 20);
        SetSpawnInCondition(AI_FLAG_FLEEING_NEVER_FIGHT_IMPOSSIBLE_ODDS, AI_TARGETING_FLEE_MASTER);
        SetAIInteger(AMOUNT_OF_HD_DIFFERENCE_TO_CHECK, -10);
        ...
        SetSpawnInCondition(AI_FLAG_COMBAT_MANY_TARGETING, AI_COMBAT_MASTER);
        SetSpawnInCondition(AI_FLAG_OTHER_COMBAT_HEAL_AT_PERCENT_NOT_AMOUNT, AI_OTHER_COMBAT_MASTER);
        SetAIInteger(AI_HEALING_US_PERCENT, 60);
        SetAIInteger(AI_HEALING_ALLIES_PERCENT, 50);
        SetAIConstant(LAG_AI_LEVEL_NO_PC_OR_ENEMY_50M, AI_LEVEL_VERY_LOW);

    }
    else if(GetLocalString(OBJECT_SELF,"Archetype") == "Mage")
    {
        SetAIInteger(AI_SPELL_LAST_TO_NEW_TARGET_CHANCE, 20);
        SetSpawnInCondition(AI_FLAG_FLEEING_NEVER_FIGHT_IMPOSSIBLE_ODDS, AI_TARGETING_FLEE_MASTER);
        SetAIInteger(AMOUNT_OF_HD_DIFFERENCE_TO_CHECK, -10);
    ...
    else if(GetLocalString(OBJECT_SELF,"Archetype") == "Fodder")
    {
        SetSpawnInCondition(AI_FLAG_FLEEING_NEVER_FIGHT_IMPOSSIBLE_ODDS, AI_TARGETING_FLEE_MASTER);
        SetAIInteger(AMOUNT_OF_HD_DIFFERENCE_TO_CHECK, -5);
    ...
    if(GetLocalInt(OBJECT_SELF,"RandomName") == 1)
    {
       string sName = "";
...
************************* [User] **********************************************/
    ...
    //VFX
    if (GetLocalString(OBJECT_SELF, "Color") == "Blue")              AI_SpawnInPermamentVisual(VFX_DUR_GLOW_BLUE);
    if (GetLocalString(OBJECT_SELF, "Color") == "Brown")             AI_SpawnInPermamentVisual(VFX_DUR_GLOW_BROWN);
    if (GetLocalString(OBJECT_SELF, "Color") == "Green")             AI_SpawnInPermamentVisual(VFX_DUR_GLOW_GREEN);
    if (GetLocalString(OBJECT_SELF, "Color") == "Grey")              AI_SpawnInPermamentVisual(VFX_DUR_GLOW_GREY);
    if (GetLocalString(OBJECT_SELF, "Color") == "Light Blue")        AI_SpawnInPermamentVisual(VFX_DUR_GLOW_LIGHT_BLUE);
    if (GetLocalString(OBJECT_SELF, "Color") == "Light Brown")       AI_SpawnInPermamentVisual(VFX_DUR_GLOW_LIGHT_BROWN);
    if (GetLocalString(OBJECT_SELF, "Color") == "Light Green")       AI_SpawnInPermamentVisual(VFX_DUR_GLOW_LIGHT_GREEN);
    if (GetLocalString(OBJECT_SELF, "Color") == "Light Orange")      AI_SpawnInPermamentVisual(VFX_DUR_GLOW_LIGHT_ORANGE);
    if (GetLocalString(OBJECT_SELF, "Color") == "Light Purple")      AI_SpawnInPermamentVisual(VFX_DUR_GLOW_LIGHT_PURPLE);
    if (GetLocalString(OBJECT_SELF, "Color") == "Light Red")         AI_SpawnInPermamentVisual(VFX_DUR_GLOW_LIGHT_RED);
    if (GetLocalString(OBJECT_SELF, "Color") == "Light Yellow")      AI_SpawnInPermamentVisual(VFX_DUR_GLOW_LIGHT_YELLOW);
    if (GetLocalString(OBJECT_SELF, "Color") == "Orange")            AI_SpawnInPermamentVisual(VFX_DUR_GLOW_ORANGE);
    if (GetLocalString(OBJECT_SELF, "Color") == "Red")               AI_SpawnInPermamentVisual(VFX_DUR_GLOW_RED);
    if (GetLocalString(OBJECT_SELF, "Color") == "White")             AI_SpawnInPermamentVisual(VFX_DUR_GLOW_WHITE);
    if (GetLocalString(OBJECT_SELF, "Color") == "Yellow")            AI_SpawnInPermamentVisual(VFX_DUR_GLOW_YELLOW);
    if (GetLocalString(OBJECT_SELF, "Natural Effect") == "Flies")    AI_SpawnInPermamentVisual(VFX_DUR_FLIES);
    if (GetLocalString(OBJECT_SELF, "Natural Effect") == "Stopped")  AI_SpawnInPermamentVisual(VFX_DUR_FREEZE_ANIMATION);
    if (GetLocalString(OBJECT_SELF, "Skin") == "Ice")                AI_SpawnInPermamentVisual(VFX_DUR_FREEZE_ANIMATION);
    if (GetLocalString(OBJECT_SELF, "Skin") == "Shadow")
    {
        AI_SpawnInPermamentVisual(VFX_DUR_PROT_SHADOW_ARMOR);
    }
    if (GetLocalString(OBJECT_SELF, "Skin") == "Statue")
    {
    SetObjectVisualTransform(OBJECT_SELF,OBJECT_VISUAL_TRANSFORM_SCALE,1.7f);
    AI_SpawnInPermamentVisual(VFX_DUR_PETRIFY);
    }
    ...
    if(GetLocalString(OBJECT_SELF,"Magical Effect") == "SpellResistance")   {AI_SpawnInPermamentVisual(VFX_DUR_MAGIC_RESISTANCE);  }
    if(GetLocalString(OBJECT_SELF,"Magical Effect") == "Invisibility")      {AI_SpawnInPermamentVisual(VFX_DUR_INVISIBILITY);      }
    if(GetLocalString(OBJECT_SELF,"Magical Effect") == "Dominated")         {ApplyEffectToObject(DURATION_TYPE_TEMPORARY,SupernaturalEffect(EffectVisualEffect(VFX_DUR_MIND_AFFECTING_DOMINATED)),OBJECT_SELF,HoursToSeconds(24));}
    if(GetLocalString(OBJECT_SELF,"Magical Effect") == "Premonition")       {AI_SpawnInPermamentVisual(VFX_DUR_PROT_PREMONITION);}
    if(GetLocalString(OBJECT_SELF,"Magical Effect") == "Teleport")          {ApplyEffectAtLocation(DURATION_TYPE_INSTANT,EffectVisualEffect(VFX_FNF_SUMMON_MONSTER_3),GetLocation(OBJECT_SELF)); }
    if(GetLocalString(OBJECT_SELF,"Magical Effect") == "Ghost")
    {
        ApplyEffectToObject(DURATION_TYPE_TEMPORARY,SupernaturalEffect(EffectVisualEffect(VFX_DUR_GHOST_SMOKE_2)),OBJECT_SELF,HoursToSeconds(24));
        ApplyEffectToObject(DURATION_TYPE_TEMPORARY,SupernaturalEffect(EffectVisualEffect(VFX_DUR_GLOW_GREY)),OBJECT_SELF,HoursToSeconds(24));
    }
    if(GetLocalString(OBJECT_SELF,"Magical Effect") == "Profane")           AI_SpawnInPermamentVisual(1835);
    if(GetLocalString(OBJECT_SELF,"Magical Effect") == "Virtue")            AI_SpawnInPermamentVisual(1978);

    if(GetLocalInt(OBJECT_SELF, "nFlying") == 1 || GetLocalInt(OBJECT_SELF,"AppearAnimation") == 1)
    {
