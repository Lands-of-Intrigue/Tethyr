...
void main()
{
    ...
    effect eVis = EffectVisualEffect(VFX_IMP_ACID_S);                        //Thing that appears on target when hit.
    effect xEff = EffectACDecrease(1, AC_DODGE_BONUS);                        //Secondary effect for flavor.
    effect eBeamF = EffectBeam(VFX_BEAM_DISINTEGRATE , oPC, BODY_NODE_HAND, TRUE);
    effect eBeamS = EffectBeam(VFX_BEAM_DISINTEGRATE , oPC, BODY_NODE_HAND, FALSE);   //Beam Effect. Edit top beam effect as well.
    ...
    else
    {
        ...
        IPSafeAddItemProperty(oMyWeapon,ItemPropertyDamageBonus(IP_CONST_DAMAGETYPE_ACID,IP_CONST_DAMAGEBONUS_2d6), TurnsToSeconds(CL), X2_IP_ADDPROP_POLICY_REPLACE_EXISTING,FALSE,TRUE);
        IPSafeAddItemProperty(oMyWeapon,ItemPropertyVisualEffect(ITEM_VISUAL_ACID), TurnsToSeconds(CL), X2_IP_ADDPROP_POLICY_REPLACE_EXISTING,FALSE,TRUE);
    }
}
