    ...
    switch (nEvent)
        {
            case X2_ITEM_EVENT_EQUIP:
                ...
                {
                    SendMessageToPC(oPC, "You feel the power of the Seldarine surge through this weapon.");
                    // Your code goes here
                    ...
                    IPSafeAddItemProperty(oItem, ItemPropertyDamageBonus(IP_CONST_DAMAGETYPE_FIRE,IP_CONST_DAMAGEBONUS_1d4), 0.0, X2_IP_ADDPROP_POLICY_KEEP_EXISTING,TRUE);
                        if (GetHasFeat(969,oPC) == TRUE)
                        {
                            IPSafeAddItemProperty(oItem, ItemPropertyEnhancementBonus(4), 0.0, X2_IP_ADDPROP_POLICY_REPLACE_EXISTING,TRUE);
                            AddItemProperty(DURATION_TYPE_PERMANENT, ItemPropertyHaste(), oItem);
                            IPSafeAddItemProperty(oItem, ItemPropertyDamageBonus(IP_CONST_DAMAGETYPE_FIRE,IP_CONST_DAMAGEBONUS_1d6), 0.0, X2_IP_ADDPROP_POLICY_REPLACE_EXISTING,TRUE);
                        }
                    }
                   if ((GetBaseItemType(oItem) == BASE_ITEM_LONGBOW))
                    ...
                    IPSafeAddItemProperty(oItem, ItemPropertyDamageBonus(IP_CONST_DAMAGETYPE_FIRE,IP_CONST_DAMAGEBONUS_1d4), 0.0, X2_IP_ADDPROP_POLICY_KEEP_EXISTING,TRUE);
                        if (GetHasFeat(969,oPC) == TRUE)
                        {
                            IPSafeAddItemProperty(oItem, ItemPropertyAttackBonus(4), 0.0, X2_IP_ADDPROP_POLICY_REPLACE_EXISTING,TRUE);
                            AddItemProperty(DURATION_TYPE_PERMANENT,ItemPropertyUnlimitedAmmo(IP_CONST_UNLIMITEDAMMO_1D6FIRE), oItem);
                        }
                    }
                  }
                break;
