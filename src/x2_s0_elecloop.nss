...
void main()
{
    ...
    //--------------------------------------------------------------------------
    int nNumDice = TE_GetCasterLevel(OBJECT_SELF,GetLastSpellCastClass())/2;
    if (nNumDice<1)
    {
        nNumDice = 1;
    }
    ...
    while (GetIsObjectValid(oTarget))
    {
        if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
        {
            ...
            if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay))
            {
                ...
                nDamage    = GetReflexAdjustedDamage(nPotential, oTarget, TE_GetSpellSaveDC(OBJECT_SELF,GetSpellId(),GetLastSpellCastClass()), SAVING_THROW_TYPE_ELECTRICITY);
