    ...
    switch (nEvent)
        {
            case X2_ITEM_EVENT_EQUIP:
                ...
                nPalLevel = GetLevelByClass(CLASS_TYPE_WIZARD,oPC);
                if (nPalLevel >= 1)
                {
                    SendMessageToPC(oPC, "The staff in your hands surges with arcane power.");
                    // Your code goes here
                    // If the spell is cast again, any previous enhancement boni are kept

                    if(nPalLevel <=4)
                    {
                        IPSafeAddItemProperty(oItem, ItemPropertyAttackBonus(1), 0.0, X2_IP_ADDPROP_POLICY_KEEP_EXISTING,TRUE);
                    }
                    else if (nPalLevel >5)
                    {
                        IPSafeAddItemProperty(oItem, ItemPropertyAttackBonus(1), 0.0, X2_IP_ADDPROP_POLICY_KEEP_EXISTING,TRUE);
