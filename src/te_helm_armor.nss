    ...
    switch (nEvent)
        {
            case X2_ITEM_EVENT_EQUIP:
                ...
                if ((nPalLevel >= 1)&&(GetHasFeat(1312,oPC)))
                {
                    SendMessageToPC(oPC, "You feel the power of your deity surge through the shillelagh in your hands.");
                    // Your code goes here
                    // If the spell is cast again, any previous enhancement boni are kept
                    if(nPalLevel < 5)
                    {
                    }
                    else if (nPalLevel >= 5 && nPalLevel < 6)
                    {
                        IPSafeAddItemProperty(oItem, ItemPropertyACBonus(1), 0.0, X2_IP_ADDPROP_POLICY_KEEP_EXISTING,TRUE);
                        IPSafeAddItemProperty(oItem, ItemPropertySkillBonus(FEAT_EPIC_SKILL_FOCUS_SPOT,5), 0.0, X2_IP_ADDPROP_POLICY_KEEP_EXISTING,TRUE);
                    }
