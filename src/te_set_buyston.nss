...
void main()
{
    ...
    if( (GetLocalInt(oPC,TRADE_STONE) >= 1 ) || ( GetItemPossessedBy(oPC,TRADE_STONE) != OBJECT_INVALID) )
    {
        object oItem = GetItemPossessedBy(oPC,TRADE_STONE); DestroyObject(oItem);
        nGold = FloatToInt(VALUE_STONE*fMerchant);
