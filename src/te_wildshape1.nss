...
AssignCommand(GetSpellTargetObject(),ClearAllActions());
    ...
    if(GetHasFeat(1140) == 1)
    {
        nPoly = 120;
    }
    else
    {
        if(nSpell == 923)//Brown Bear
        {
            nPoly = 126;
        }
        else if (nSpell == 924)//Panther
        {
            nPoly = 129;
        }
        else if (nSpell == 925)//Wolf
        {
            nPoly = 128;
        }
        else if (nSpell == 926)//Boar
        {
            nPoly = 127;
        }
        else if (nSpell == 927)//Badger
        {
            nPoly = 130;
        }
    }
    ...
    ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectRegenerate(2,6.0), OBJECT_SELF, RoundsToSeconds(nDuration));
