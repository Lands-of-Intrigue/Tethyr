...
void main()
{
    ...
    {
        oMount = HorseCreateHorse("te_hrs_black",lPC,oPC,"",540);
    }
    else if(GetIsObjectValid(oMediumBard) == TRUE)
    {
        oMount = HorseCreateHorse("te_hrs_black",lPC,oPC,"",539);
    }
    else if(GetIsObjectValid(oLightBard) == TRUE)
    {
        oMount = HorseCreateHorse("te_hrs_black",lPC,oPC,"",538);
    }
    else
    {
        oMount = HorseCreateHorse("te_hrs_black",lPC,oPC,"",536);
    }

    SetLocalInt(oItem,"te_hrs_black",0);
