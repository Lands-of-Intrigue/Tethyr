...
// End of Spell Cast Hook
    ...
    if (spellsIsTarget(oFirstTarget, SPELL_TARGET_SELECTIVEHOSTILE, OBJECT_SELF))
    {
        ...
        //Make an SR Check
        if (!MyResistSpell(OBJECT_SELF, oFirstTarget)|| GetTag(oTarget) == "te_gol_flesh")
        {
            //Adjust damage via Reflex Save or Evasion or Improved Evasion
            nDamStrike = GetReflexAdjustedDamage(nDamage, oFirstTarget, TE_GetSpellSaveDC(OBJECT_SELF,GetSpellId(),GetLastSpellCastClass()), SAVING_THROW_TYPE_ELECTRICITY);
    ...
    while (GetIsObjectValid(oTarget) && nCnt < nCasterLevel)
    {
        ...
        if (oTarget != oFirstTarget && spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, OBJECT_SELF) && oTarget != OBJECT_SELF)
        {
            ...
            if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay))
            {
                ...
                //Adjust damage via Reflex Save or Evasion or Improved Evasion
                nDamStrike = GetReflexAdjustedDamage(nDamage, oTarget, TE_GetSpellSaveDC(OBJECT_SELF,GetSpellId(),GetLastSpellCastClass()), SAVING_THROW_TYPE_ELECTRICITY);
