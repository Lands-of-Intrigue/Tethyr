...
int ApplyPrismaticEffect(int nEffect, object oTarget, int nDC);
        ...
        if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
        {
            ...
            if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay) && (oTarget != OBJECT_SELF))
            {
                ...
                if(nRandom == 8)
                {
                    ...
                    nVisual = ApplyPrismaticEffect(Random(7) + 1, oTarget,nDC);
                    nVisual = ApplyPrismaticEffect(Random(7) + 1, oTarget,nDC);
                }
                else
                {
                    //Get the visual effect
                    nVisual = ApplyPrismaticEffect(nRandom, oTarget, TE_GetCasterLevel(OBJECT_SELF,GetLastSpellCastClass()));
                }
