...
void main()
{
    ...
    int nBlackcoatlevel = GetLevelByClass(51);
    object oTurner = OBJECT_SELF;
    int nTotalLevel =  GetHitDice(oTurner);
    ...
    // add the blackcoat case
    if(nBlackcoatlevel > 0)
    {
        nClassLevel += nBlackcoatlevel;
        nTurnLevel  += nBlackcoatlevel;
    }

    // if Lich and good alignment you get Turning power as if you where a pure Cleric as per the Arch Lich Template
    if(GetPCAffliction(oTurner) == 7 && GetAlignmentGoodEvil(oTurner) == ALIGNMENT_GOOD)
    {
        nTurnLevel  = nTotalLevel;
        nClassLevel = nTotalLevel;
    }
    ...
    SendMessageToPC(oTurner, "nTurnLevel: " + IntToString(nTurnLevel));
    SendMessageToPC(oTurner, "nTurnHD: " + IntToString(nTurnHD));
    ...
    while(GetIsObjectValid(oTarget) && nHDCount < nTurnHD && GetDistanceToObject(oTarget) <= 20.0)
    {
        if(!GetIsFriend(oTarget))
        {
            ...
            if(nHD <= nTurnLevel && nHD <= (nTurnHD - nHDCount))
            {
                ...
                // * if creature is a good Lich they cant be turned
                else if (GetPCAffliction(oTurner) == 7 && GetAlignmentGoodEvil(oTurner) == ALIGNMENT_GOOD)
                {
                    bValid = FALSE;
                }
