...
void main()
{
    ...
    if( (GetLocalInt(oPC,TRADE_GARLIC) >= 10 ) || ( GetItemPossessedBy(oPC,TRADE_GARLIC) != OBJECT_INVALID) )
    {
        object oItem = GetFirstItemInInventory(oPC);
        int nCount = 0;
        while (oItem != OBJECT_INVALID)
        {
            if((GetTag(oItem) == TRADE_GARLIC) && nCount < 10)
            {
                DestroyObject(oItem);
                nCount += 1;
            }
        ...
        nGold = FloatToInt(VALUE_GARLIC*fMerchant);
