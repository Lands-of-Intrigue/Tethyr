...
void main()
{
    ...
    if( (GetLocalInt(oPC,TRADE_SKINSOFT) >= 10 ) || ( GetItemPossessedBy(oPC,TRADE_SKINSOFT) != OBJECT_INVALID) )
    {
        object oItem = GetFirstItemInInventory(oPC);
        int nCount = 0;
        while (oItem != OBJECT_INVALID)
        {
            if((GetTag(oItem) == TRADE_SKINSOFT) && nCount < 10)
            {
                DestroyObject(oItem);
                nCount += 1;
            }
        ...
        nGold = FloatToInt(VALUE_SKINSOFT*fMerchant);
