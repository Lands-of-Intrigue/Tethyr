...
// -------------------------------------------
            ...
            //
            // Spawn if the PC is bearing a Light
            else if (nCheckPCs == 10)
            {
                if(     GetLocalInt(oPC, LIGHT_VALUE)
                    ||  GetHasSpellEffect(SPELL_LIGHT,oPC)
                    ||  GetHasSpellEffect(SPELL_CONTINUAL_FLAME,oPC)
                  )
                    nProcessSpawn = TRUE;
            }
            //
