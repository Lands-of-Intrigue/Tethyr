...
// End of Spell Cast Hook
    ...
    while (GetIsObjectValid(oTarget))
    {
        ...
        if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF) && oTarget != OBJECT_SELF)
        {
            ...
            {
                if(GetRacialType(oTarget) != RACIAL_TYPE_CONSTRUCT && GetIsUndead(oTarget) != TRUE)
                {
                    ...
                    if(/*Fort Save*/ MySavingThrow(SAVING_THROW_FORT, oTarget, TE_GetSpellSaveDC(OBJECT_SELF,GetSpellId(),GetLastSpellCastClass()), SAVING_THROW_TYPE_NONE, OBJECT_SELF, fDelay))
                    {
                        nDamage = nDamage/2;
                    }
